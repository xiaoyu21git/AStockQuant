...
std::optional<int> commonStandardizationIndex(const QString& rawValue)
{
    ...
    if (normalized == QStringLiteral("percentile") || normalized == QStringLiteral("rank")) {
        return static_cast<int>(factor::StandardizationMethod::Percentile);
    }
    return std::nullopt;
}
...
std::optional<int> macroDimensionIndex(const QString& rawValue)
{
    const QString normalized = rawValue.trimmed().toLower();
    if (normalized == QStringLiteral("growth")) return static_cast<int>(factor::MacroDimension::GROWTH);
    if (normalized == QStringLiteral("inflation")) return static_cast<int>(factor::MacroDimension::INFLATION);
    if (normalized == QStringLiteral("credit")) return static_cast<int>(factor::MacroDimension::CREDIT);
    if (normalized == QStringLiteral("rates")) return static_cast<int>(factor::MacroDimension::RATES);
    if (normalized == QStringLiteral("policy")) return static_cast<int>(factor::MacroDimension::POLICY);
    if (normalized == QStringLiteral("risk_appetite")) return static_cast<int>(factor::MacroDimension::RISK_APPETITE);
    return std::nullopt;
}

std::optional<int> macroIndicatorIndex(const QString& rawValue)
{
    const QString normalized = rawValue.trimmed().toLower();
    if (normalized == QStringLiteral("industrial_added_value_yoy")) return static_cast<int>(factor::MacroIndicator::INDUSTRIAL_ADDED_VALUE_YOY);
    if (normalized == QStringLiteral("manufacturing_pmi")) return static_cast<int>(factor::MacroIndicator::MANUFACTURING_PMI);
    if (normalized == QStringLiteral("gdp_yoy")) return static_cast<int>(factor::MacroIndicator::GDP_YOY);
    if (normalized == QStringLiteral("cpi_yoy")) return static_cast<int>(factor::MacroIndicator::CPI_YOY);
    if (normalized == QStringLiteral("ppi_yoy")) return static_cast<int>(factor::MacroIndicator::PPI_YOY);
    if (normalized == QStringLiteral("m2_yoy")) return static_cast<int>(factor::MacroIndicator::M2_YOY);
    if (normalized == QStringLiteral("social_financing_stock_yoy")) return static_cast<int>(factor::MacroIndicator::SOCIAL_FINANCING_STOCK_YOY);
    if (normalized == QStringLiteral("m1_m2_spread")) return static_cast<int>(factor::MacroIndicator::M1_M2_SPREAD);
    if (normalized == QStringLiteral("ten_year_bond_yield")) return static_cast<int>(factor::MacroIndicator::TEN_YEAR_BOND_YIELD);
    if (normalized == QStringLiteral("shibor_3m")) return static_cast<int>(factor::MacroIndicator::SHIBOR_3M);
    if (normalized == QStringLiteral("lpr_1y")) return static_cast<int>(factor::MacroIndicator::LPR_1Y);
    if (normalized == QStringLiteral("reserve_requirement_ratio")) return static_cast<int>(factor::MacroIndicator::RESERVE_REQUIREMENT_RATIO);
    if (normalized == QStringLiteral("aa_credit_spread")) return static_cast<int>(factor::MacroIndicator::AA_CREDIT_SPREAD);
    if (normalized == QStringLiteral("vix_proxy")) return static_cast<int>(factor::MacroIndicator::VIX_PROXY);
    return std::nullopt;
}

std::optional<int> technicalIndicatorIndex(const QString& rawValue)
{
    const QString normalized = rawValue.trimmed().toLower();
    if (normalized == QStringLiteral("rsi")) return static_cast<int>(factor::TechnicalIndicator::RSI);
    if (normalized == QStringLiteral("macd")) return static_cast<int>(factor::TechnicalIndicator::MACD);
    if (normalized == QStringLiteral("ma")) return static_cast<int>(factor::TechnicalIndicator::MA);
    if (normalized == QStringLiteral("ema")) return static_cast<int>(factor::TechnicalIndicator::EMA);
    if (normalized == QStringLiteral("boll")) return static_cast<int>(factor::TechnicalIndicator::BOLL);
    if (normalized == QStringLiteral("kdj")) return static_cast<int>(factor::TechnicalIndicator::KDJ);
    if (normalized == QStringLiteral("atr")) return static_cast<int>(factor::TechnicalIndicator::ATR);
    if (normalized == QStringLiteral("obv")) return static_cast<int>(factor::TechnicalIndicator::OBV);
    if (normalized == QStringLiteral("vwap")) return static_cast<int>(factor::TechnicalIndicator::VWAP);
    if (normalized == QStringLiteral("volume_ratio")) return static_cast<int>(factor::TechnicalIndicator::VOLUME_RATIO);
    if (normalized == QStringLiteral("turnover_stability")) return static_cast<int>(factor::TechnicalIndicator::TURNOVER_STABILITY);
    return std::nullopt;
}
...
foundation::json::JsonFacade parseRuntimeCompatibleTestConfig(const char* configJson)
{
    ...
    if (root.contains(QStringLiteral("calculation")) && root.value(QStringLiteral("calculation")).isObject()) {
        ...
        normalizeEnumArrayField(calculation, "macroDimensions", macroDimensionIndex);
        normalizeEnumArrayField(calculation, "macroIndicators", macroIndicatorIndex);
        normalizeEnumArrayField(calculation, "technicalIndicators", technicalIndicatorIndex);
...
public:
    ...
    static void setDatabase(FactorBacktestController& controller,
                            const std::shared_ptr<astock::database::QtMySQLDatabase>& database)
    {
        controller.m_database = database;
    }
    ...
    {
        return controller.buildBacktestConfig(
            resolvedInstanceId,
            groupText,
            startDate,
            endDate,
            controller.m_dataSourceMode,
            controller.m_selectedDatasetId,
            controller.m_selectedDatasetBenchmarkMetadata,
            controller.m_selectedStockPoolSymbols,
            controller.m_backtestRuntimeParams,
            static_cast<int>(controller.m_batchFactorIds.size()),
            static_cast<int>(controller.m_threadPool ? controller.m_threadPool->getWorkerCount() : 0));
    }
    ...
    static void primeQueuedBatchLaunchState(FactorBacktestController& controller,
                                            const QVariantList& factorIds)
    {
        controller.m_cancelRequested.store(false);
        controller.m_isRunning = true;
        controller.m_progress = 0;
        controller.m_status = QStringLiteral("正在提交回测任务");
        controller.m_batchFactorIds = factorIds;
        controller.m_batchResultMaps.clear();
        controller.m_batchResultMaps.resize(static_cast<size_t>(factorIds.size()));
        controller.m_pendingGroupText = QStringLiteral("5组");
        controller.m_pendingStartDate = QStringLiteral("2024-01-02");
        controller.m_pendingEndDate = QStringLiteral("2024-01-26");
        controller.m_pendingDataSourceMode = controller.m_dataSourceMode;
        controller.m_pendingDatasetId = controller.m_selectedDatasetId;
        controller.m_pendingDatasetBenchmarkMetadata = controller.m_selectedDatasetBenchmarkMetadata;
        controller.m_pendingStockPoolSymbols = controller.m_selectedStockPoolSymbols;
    ...
    static void launchNextBacktestTask(FactorBacktestController& controller)
    {
        controller.launchNextBacktestTask();
    }

    static void pollBacktestProgress(FactorBacktestController& controller)
    {
        controller.pollBacktestProgress();
    }

    static void cleanupDetachedBacktestTasks(FactorBacktestController& controller,
                                             bool waitForCompletion)
    {
        controller.cleanupDetachedBacktestTasks(waitForCompletion);
    }

    static void setPendingLaunchProgress(FactorBacktestController& controller,
                                         size_t taskIndex,
                                         int progress,
                                         const QString& step)
    {
    ...
    static void primeSupportMapRequestState(FactorBacktestController& controller,
                                            int requestSeq,
                                            int appliedSeq,
                                            bool inFlight,
                                            const QVariantMap& supportMapCache)
    {
        controller.m_supportMapRequestSeq = requestSeq;
        controller.m_supportMapAppliedSeq = appliedSeq;
        controller.m_supportMapRequestInFlight = inFlight;
        controller.m_factorSupportMapCache = supportMapCache;
    }
};
...
class FactorBacktestExecutorTestAccess
{
public:
    using ExecutionMarketContext = FactorBacktestExecutor::ExecutionMarketContext;

    static bool applyFactorWarmupToMarketContext(FactorBacktestExecutor& executor,
...
QVariantMap makeGroupResult(int groupId, double groupReturn)
{
    ...
    group["returnRate"] = groupReturn;
    group["annualizedReturn"] = groupReturn * 252.0;
    group["stockCount"] = 12;
...
QVariantMap resultMetricSections(const QVariantMap& result)
{
    return result.value("metrics").toMap();
}

QVariantMap factorQualityMetricSection(const QVariantMap& result)
{
    return resultMetricSections(result).value("factorQuality").toMap();
}

QVariantMap researchMetricSection(const QVariantMap& result)
{
    return resultMetricSections(result).value("research").toMap();
}

QVariantMap executionMetricSection(const QVariantMap& result)
{
    return resultMetricSections(result).value("execution").toMap();
}

QVariantMap icMetricSection(const QVariantMap& result)
{
    return resultMetricSections(result).value("ic").toMap();
}

QVariantList groupMetricSection(const QVariantMap& result)
{
    return resultMetricSections(result).value("groups").toList();
}
                                   ...
                                   int executionTime)
{
    ...
    result["metrics"] = QVariantMap{
        {"research", QVariantMap{{"spreadReturn", spreadReturn}, {"dataCoverage", 0.94}}},
        {"execution", QVariantMap{{"sharpeRatio", 1.12}, {"turnoverRate", 0.18}}},
        {"ic", QVariantMap{{"value", icValue}, {"ir", 0.45}, {"positiveRate", 0.63}}},
        {"groups", QVariantList{makeGroupResult(1, 0.031), makeGroupResult(2, 0.014)}}};
    return result;
}
...
std::shared_ptr<factor::BaseFactor> createRuntimeFactorInstance(const factor::FactorInstanceInfo& info)
{
    switch (info.factorType) {
    case factor::FactorType::MOMENTUM:
        return factor::MomentumFactor::create(info, nullptr);
    case factor::FactorType::VALUE:
        return factor::ValueFactor::create(info, nullptr);
    case factor::FactorType::QUALITY:
        return factor::QualityFactor::create(info, nullptr);
    case factor::FactorType::SIZE:
        return factor::SizeFactor::create(info, nullptr);
    case factor::FactorType::LOW_VOLATILITY:
        return factor::LowVolFactor::create(info, nullptr);
    case factor::FactorType::GROWTH:
        return factor::GrowthFactor::create(info, nullptr);
    case factor::FactorType::DIVIDEND:
        return factor::DividendFactor::create(info, nullptr);
    case factor::FactorType::TECHNICAL:
        return factor::TechnicalFactor::create(info, nullptr);
    case factor::FactorType::LIQUIDITY:
        return factor::LiquidityFactor::create(info, nullptr);
    case factor::FactorType::MACRO:
        return factor::MacroFactor::create(info, nullptr);
    case factor::FactorType::INDUSTRY:
        return factor::IndustryFactor::create(info, nullptr);
    case factor::FactorType::SENTIMENT:
        return factor::SentimentFactor::create(info, nullptr);
    case factor::FactorType::CUSTOM:
        return factor::CustomFactor::create(info, nullptr);
    default:
        return nullptr;
    }
}

std::vector<std::string> jsonStringArray(const foundation::json::JsonFacade& value)
{
    std::vector<std::string> values;
    values.reserve(value.size());
    for (size_t index = 0; index < value.size(); ++index) {
        values.push_back(value.at(index).asString());
    }
    return values;
}
...
int expectedConfigurableLookbackWindow(const foundation::json::JsonFacade& calculation)
{
    if (calculation.has("lookbackWindow")) {
        return calculation.get("lookbackWindow").asInt();
    }
    return 252;
}
                                        ...
                                        int executionTimeMs)
{
    ...
    result.actualStartDate = "2024-01-01";
    result.warmupTrimmedTradingDays = 0;
...
TEST(FactorBacktestRegressionTest, LoadMissingResultFileKeepsExistingRestoredState)
{
    ...
    const QVariantMap previousMetrics = controller.resultMetrics();
    ...
    EXPECT_EQ(controller.resultMetrics(), previousMetrics);
}
...
TEST(FactorBacktestRegressionTest, LoadSingleFactorResultReplacesPriorBatchMetadata)
{
    ...
    EXPECT_EQ(controller.resultMetrics().value("groups").toList().size(), 2);
    EXPECT_DOUBLE_EQ(controller.resultMetrics().value("ic").toMap().value("value").toDouble(), 0.031);
    EXPECT_DOUBLE_EQ(controller.resultMetrics().value("research").toMap().value("spreadReturn").toDouble(), 0.012);
}
...
TEST(FactorBacktestRegressionTest, FinalizeBacktestSuccessEmitsCompletedStateForSingleFactorBatch)
{
    ...
    EXPECT_EQ(emittedResult.value("metrics").toMap().value("groups").toList().size(), 5);
}
...
TEST(FactorBacktestRegressionTest, StartBacktestWithCachedResultReachesCompletionSignal)
{
    ...
    EXPECT_EQ(completedResult.value("metrics").toMap().value("groups").toList().size(), 5);
    EXPECT_GT(controller.resultMetrics().value("research").toMap().value("spreadReturn").toDouble(), 0.0);
    EXPECT_GT(controller.resultMetrics().value("ic").toMap().value("value").toDouble(), 0.9);

    cache.clearAllCache();
}

TEST(FactorBacktestRegressionTest, BatchLaunchStartsOnlyOnePendingBacktestTaskAtATime)
{
    FactorBacktestController controller;
...
TEST(FactorBacktestRegressionTest, PollBacktestProgressUsesExecutorProgressForRunningFactor)
{
    ...
    FactorBacktestControllerTestAccess::primeQueuedBatchLaunchState(
        controller,
        QVariantList{QStringLiteral("factor_running")});

    const QString instanceId = QStringLiteral("real_progress_factor");
    auto instanceManager = std::make_shared<factor::FactorInstanceManager>(nullptr, nullptr);
    const char* configJson = R"JSON({
        "factorType": "custom",
        "majorCategory": "自定义因子",
    ...
    auto threadPool = std::make_shared<foundation::thread::ThreadPoolExecutor>(4);
    auto executor = std::make_shared<FactorBacktestExecutor>(instanceManager, threadPool, nullptr);

    BacktestConfig config = makeCachedBacktestConfig(instanceId.toStdString());
    config.startDate = "2024-01-02";
    config.endDate = "2024-09-30";
    config.forwardDays = 1;
    config.numGroups = 5;
    ...
    for (int index = 0; index < 200; ++index) {
        symbols.push_back(QStringLiteral("SYM%1").arg(index, 3, 10, QLatin1Char('0')).toStdString());
    }
    for (QDate date = QDate(2024, 1, 2); date <= QDate(2024, 9, 30); date = date.addDays(1)) {
        if (date.dayOfWeek() >= 6) {
            continue;
        }
        const std::string tradeDate = date.toString(QStringLiteral("yyyy-MM-dd")).toStdString();
        for (size_t symbolIndex = 0; symbolIndex < symbols.size(); ++symbolIndex) {
            const double basePrice = 10.0 + static_cast<double>(symbolIndex) * 0.05;
            const double dayDrift = static_cast<double>(date.dayOfYear()) * 0.01;
            config.cachedBars.push_back({
                symbols[symbolIndex],
                tradeDate,
                basePrice + dayDrift + 0.2,
                {{"open", basePrice + dayDrift}}
            });
        }
    }
    ...
    QElapsedTimer timer;
    timer.start();
    bool observedMidProgress = false;
    while (timer.elapsed() < 5000) {
        FactorBacktestControllerTestAccess::pollBacktestProgress(controller);
        if (controller.progress() > 0 && controller.progress() < 100) {
            observedMidProgress = true;
            break;
        }
        if (controller.status() == QStringLiteral("回测完成")) {
            break;
        }
    }

    EXPECT_TRUE(observedMidProgress);
    EXPECT_GT(controller.progress(), 0);
    EXPECT_LT(controller.progress(), 100);
    EXPECT_FALSE(controller.status().trimmed().isEmpty());

    controller.cancelBacktest();
...
TEST(FactorBacktestRegressionTest, StartBacktestWithFactorsDoesNotRequirePreparedSupportMapCache)
{
    ...
    const QString instanceId = QStringLiteral("factor_requires_prepared_support_map");
    const char* configJson = R"JSON({
        "factorType": "momentum",
        "majorCategory": "动量因子",
        "dataRequirements": {
...
TEST(FactorBacktestRegressionTest, StartBacktestWithFactorsEmitsStartedOncePerBatch)
{
    FactorBacktestController controller;
    controller.setSelectedDatasetId(48);

    const QVariantMap supportMap{
        {QStringLiteral("factor_a"), QVariantMap{{QStringLiteral("factorId"), QStringLiteral("factor_a")}, {QStringLiteral("instanceId"), QStringLiteral("factor_a")}, {QStringLiteral("supported"), true}}},
        {QStringLiteral("factor_b"), QVariantMap{{QStringLiteral("factorId"), QStringLiteral("factor_b")}, {QStringLiteral("instanceId"), QStringLiteral("factor_b")}, {QStringLiteral("supported"), true}}}
    };
    ...
    int startedCount = 0;
    QObject::connect(&controller,
                     &FactorBacktestController::backtestStarted,
    ...
    QElapsedTimer timer;
    timer.start();
    while (timer.elapsed() < 5000 && startedCount == 0 && controller.isRunning()) {
        FactorBacktestControllerTestAccess::pollBacktestProgress(controller);
        QCoreApplication::processEvents();
    }
    FactorBacktestControllerTestAccess::cleanupDetachedBacktestTasks(controller, true);
...
TEST(FactorBacktestRegressionTest, StartBacktestWithFactorsDoesNotEmitStartedWhenDateMissing)
{
    FactorBacktestController controller;
    controller.setSelectedDatasetId(48);
    ...
    QElapsedTimer timer;
    timer.start();
    while (timer.elapsed() < 5000 && failureMessage.isEmpty() && controller.isRunning()) {
        FactorBacktestControllerTestAccess::pollBacktestProgress(controller);
        QCoreApplication::processEvents();
    }
    FactorBacktestControllerTestAccess::cleanupDetachedBacktestTasks(controller, true);
    ...
    EXPECT_EQ(failureMessage, QStringLiteral("回测开始日期缺失，禁止使用默认兜底日期"));
}

TEST(FactorBacktestRegressionTest, SupportMapStaleResultIsRejectedAfterSelectionInvalidation)
{
    FactorBacktestController controller;

    const QVariantMap cachedSupportMap{
        {QStringLiteral("old_factor"), QVariantMap{{QStringLiteral("supported"), true}}}
    };
    FactorBacktestControllerTestAccess::primeSupportMapRequestState(
        controller,
        1,
        1,
        true,
    ...
    EXPECT_TRUE(controller.factorSupportMapCache().isEmpty());

    const QVariantMap staleSupportMap{
        {QStringLiteral("stale_factor"), QVariantMap{{QStringLiteral("supported"), true}}}
    };
    EXPECT_FALSE(controller.handleFactorSupportMapReady(1, staleSupportMap));
    EXPECT_TRUE(controller.factorSupportMapCache().isEmpty());
    ...
    const QVariantMap latestSupportMap{
        {QStringLiteral("latest_factor"), QVariantMap{{QStringLiteral("supported"), true}}}
    };
...
TEST(FactorBacktestRegressionTest, BuildFactorSupportMapReusesPersistedUnsupportedFactorsWithoutRechecking)
{
    auto& cache = DataServiceCache::getInstance();
    ASSERT_TRUE(cache.initializeCache());
    cache.clearAllCache();
    ...
    for (int day = 2; day <= 6; ++day) {
        const QString tradeDate = QStringLiteral("2024-01-%1").arg(day, 2, 10, QLatin1Char('0'));
        for (int symbolIndex = 0; symbolIndex < stockCodes.size(); ++symbolIndex) {
            rows.append(QVariantMap{{"symbol", stockCodes.at(symbolIndex)},
                                    {"trade_date", tradeDate},
                                    {"close", 10.0 + static_cast<double>(day) + static_cast<double>(symbolIndex)}});
        }
    }
    ...
    const factor::FactorInstanceInfo instanceInfo = makeFactorInstanceInfo(
        QStringLiteral("factor_insufficient_history"),
        QString::fromUtf8("动量因子"),
        config);
    ...
    auto configureController = [&](FactorBacktestController& controller) {
        ...
        FactorBacktestControllerTestAccess::setRequiredWarmupTradingDays(
            controller,
            QStringLiteral("factor_insufficient_history"),
            10);
    };
    ...
    configureController(firstController);
    const QVariantMap firstSupportMap = firstController.buildFactorSupportMap(
        QVariantList{QStringLiteral("factor_insufficient_history")},
        QStringLiteral("2024-01-02"),
        QStringLiteral("2024-01-06"),
        cacheSnapshot);
    const QVariantMap firstSupportInfo = firstSupportMap.value(QStringLiteral("factor_insufficient_history")).toMap();
    EXPECT_FALSE(firstSupportInfo.value(QStringLiteral("supported")).toBool());
    EXPECT_EQ(firstSupportInfo.value(QStringLiteral("category")).toString(), QStringLiteral("insufficient-history"));
    EXPECT_EQ(factorInstanceCreateCount, 1);
    ...
    const QVariantMap secondSupportMap = secondController.buildFactorSupportMap(
        ...
        QStringLiteral("2024-01-06"),
        cacheSnapshot);
    const QVariantMap secondSupportInfo = secondSupportMap.value(QStringLiteral("factor_insufficient_history")).toMap();
    EXPECT_FALSE(secondSupportInfo.value(QStringLiteral("supported")).toBool());
    EXPECT_EQ(secondSupportInfo.value(QStringLiteral("category")).toString(), QStringLiteral("insufficient-history"));
    EXPECT_EQ(factorInstanceCreateCount, 1);
...
TEST(FactorBacktestRegressionTest, BuildResultMapUsesResearchOnlyGroupMetricsWithoutFabricatedRiskValues)
{
    ...
    const QVariantList groups = groupMetricSection(resultMap);
...
TEST(FactorBacktestRegressionTest, BuildResultMapIncludesBenchmarkDerivedSummaryMetrics)
{
    ...
    const QVariantMap execution = executionMetricSection(resultMap);
    EXPECT_DOUBLE_EQ(execution.value("benchmarkAnnualReturn").toDouble(), 0.12);
    EXPECT_DOUBLE_EQ(execution.value("excessAnnualReturn").toDouble(), 0.07);
    EXPECT_DOUBLE_EQ(execution.value("trackingError").toDouble(), 0.14);
    EXPECT_DOUBLE_EQ(execution.value("informationRatio").toDouble(), 0.5);
    EXPECT_DOUBLE_EQ(execution.value("alpha").toDouble(), 0.03);
    EXPECT_DOUBLE_EQ(execution.value("beta").toDouble(), 0.85);
}

TEST(FactorBacktestRegressionTest, BuildResultMapIncludesFactorQualityMetrics)
{
    FactorBacktestController controller;
    BacktestResult result = makeCachedExecutorResult("factor_quality_instance", 0.19, 11);
    ...
    result.factorMetrics.icHalfLife = 5;
    result.factorMetrics.annualTurnover = 1.37;
    result.factorMetrics.costAdjustedSharpe = 1.8;
    result.factorMetrics.alpha = 0.03;
    result.factorMetrics.icTStat = 4.5;
    result.factorMetrics.monthlyWinRate = 0.67;
    ...
    const QVariantMap firstGroupChart = groupCharts.first().toMap();
    EXPECT_EQ(firstGroupChart.value("key").toString(), QStringLiteral("groupAnnualReturns"));
    EXPECT_EQ(firstGroupChart.value("title").toString(), QStringLiteral("分组年化梯度"));
    EXPECT_TRUE(firstGroupChart.value("isPercent").toBool());
    ASSERT_EQ(firstGroupChart.value("series").toList().size(), 5);

    const QVariantList groupAnnualReturns = factorQuality.value("groupAnnualReturns").toList();
    const QVariantList groupSharpes = factorQuality.value("groupSharpes").toList();
    ASSERT_EQ(groupAnnualReturns.size(), 5);
    ASSERT_EQ(groupSharpes.size(), 5);
    EXPECT_DOUBLE_EQ(groupAnnualReturns.first().toDouble(), 0.21);
...
TEST(FactorBacktestRegressionTest, BuildResultMapIncludesActualStartDateAndWarmupTrim)
{
    ...
    BacktestResult result = makeCachedExecutorResult("factor_quality_instance", 0.19, 11);
    result.config.startDate = "2024-01-01";
    result.actualStartDate = "2024-01-05";
    result.warmupTrimmedTradingDays = 2;
    ...
    EXPECT_EQ(execution.value("actualStartDate").toString(), QStringLiteral("2024-01-05"));
    EXPECT_EQ(execution.value("warmupTrimmedTradingDays").toInt(), 2);
...
TEST(FactorBacktestRegressionTest, MomentumFactorBoundaryRulesIncludeSkipRecentWarmup)
{
    MomentumFactor factor;
    MomentumFactor::Params params;
    params.window = 20;
    ...
    const factor::BoundaryRules rules = factor.getBoundaryRules();

    EXPECT_EQ(rules.minDataPoints, 26);
}

TEST(FactorBacktestRegressionTest, MomentumFactorSerializedBoundaryRulesIncludeSkipRecentWarmup)
{
    const auto instanceInfo = makeFactorInstanceInfo(
        QStringLiteral("momentum_serialized_boundary_skiprecent_instance"),
        QString::fromUtf8("动量因子"),
        R"JSON({
            "factorType": "momentum",
            "calculation": {
    ...
    const auto serialized = factorInstance->toJson();
    ASSERT_TRUE(serialized.has("boundaryRules"));
    const auto boundaryRules = serialized.get("boundaryRules");
    ASSERT_TRUE(boundaryRules.has("minDataPoints"));
    EXPECT_EQ(boundaryRules.get("minDataPoints").asInt(), 26);
}

TEST(FactorBacktestRegressionTest, ClassicFactorBoundaryRulesUseRuntimeParametersAndPreserveConfiguredHandling)
{
    struct ClassicBoundaryCase {
        QString instanceId;
        QString factorType;
        const char* configJson;
        int expectedMinDataPoints;
        factor::NewStockHandling expectedHandleNewStock;
        factor::SuspendedHandling expectedHandleSuspended;
        factor::DelistedHandling expectedHandleDelisted;
        factor::OutlierHandling expectedHandleOutliers;
    };

    const std::vector<ClassicBoundaryCase> cases{
        {QStringLiteral("value_runtime_boundary_contract"),
         ...
         3,
         factor::NewStockHandling::INCLUDE,
         factor::SuspendedHandling::SET_NULL,
         factor::DelistedHandling::EXCLUDE,
         factor::OutlierHandling::KEEP},
        {QStringLiteral("size_runtime_boundary_contract"),
         QStringLiteral("size"),
         R"JSON({
            "factorType": "size",
            "calculation": {
         ...
         4,
         factor::NewStockHandling::INCLUDE,
         factor::SuspendedHandling::EXCLUDE,
         factor::DelistedHandling::EXCLUDE,
         factor::OutlierHandling::EXCLUDE},
        {QStringLiteral("quality_runtime_boundary_contract"),
         ...
         factor::DelistedHandling::EXCLUDE,
         factor::OutlierHandling::EXCLUDE},
        {QStringLiteral("lowvol_runtime_boundary_contract"),
         ...
         factor::DelistedHandling::EXCLUDE,
         factor::OutlierHandling::KEEP},
        {QStringLiteral("momentum_runtime_boundary_contract"),
    ...
    for (const auto& testCase : cases) {
        SCOPED_TRACE(testCase.instanceId.toStdString());
        const auto instanceInfo = makeFactorInstanceInfo(testCase.instanceId, testCase.factorType, testCase.configJson);
        auto factorInstance = createRuntimeFactorInstance(instanceInfo);
        ASSERT_NE(factorInstance, nullptr);

        const factor::BoundaryRules rules = factorInstance->getBoundaryRules();
        EXPECT_EQ(rules.minDataPoints, testCase.expectedMinDataPoints);
        EXPECT_EQ(rules.handleNewStock, testCase.expectedHandleNewStock);
        EXPECT_EQ(rules.handleSuspended, testCase.expectedHandleSuspended);
        EXPECT_EQ(rules.handleDelisted, testCase.expectedHandleDelisted);
        EXPECT_EQ(rules.handleOutliers, testCase.expectedHandleOutliers);

        const auto serialized = factorInstance->toJson();
        ASSERT_TRUE(serialized.has("boundaryRules"));
        const auto boundaryRules = serialized.get("boundaryRules");
        EXPECT_EQ(boundaryRules.get("minDataPoints").asInt(), rules.minDataPoints);
        EXPECT_EQ(boundaryRules.get("handleNewStock").asInt(), static_cast<int>(rules.handleNewStock));
        EXPECT_EQ(boundaryRules.get("handleSuspended").asInt(), static_cast<int>(rules.handleSuspended));
        EXPECT_EQ(boundaryRules.get("handleDelisted").asInt(), static_cast<int>(rules.handleDelisted));
        EXPECT_EQ(boundaryRules.get("handleOutliers").asInt(), static_cast<int>(rules.handleOutliers));
    }
}

TEST(FactorBacktestRegressionTest, ConfigurableFactorsDeriveRuntimeContractsFromActualParameters)
{
    struct ConfigurableContractCase {
        QString instanceId;
        QString factorType;
        const char* configJson;
        std::vector<std::string> expectedRequiredFields;
        factor::SourceTable expectedSourceTable;
        int expectedMinDataPoints;
    };

    const std::vector<ConfigurableContractCase> cases{
        {QStringLiteral("growth_runtime_contract"),
         QStringLiteral("growth"),
         R"JSON({
            "factorType": "growth",
            "calculation": {
                "growthMetrics": ["revenue_growth", "sue"],
                "growthWeights": [0.5, 0.5],
                "laggedEnabled": true,
                "lagPeriods": 2,
                "neutralizationEnabled": true
            }
         })JSON",
         {"total_revenue", "eps", "industry_code", "market_cap"},
         factor::SourceTable::UNKNOWN,
         7},
        {QStringLiteral("liquidity_runtime_contract"),
         ...
         {"turnover_rate", "industry_code", "market_cap"},
         factor::SourceTable::UNKNOWN,
         20},
        {QStringLiteral("dividend_runtime_contract"),
         QStringLiteral("dividend"),
         R"JSON({
            "factorType": "dividend",
            "calculation": {
                "dividendMetrics": ["dividend_yield", "dividend_stability"],
                "neutralizationEnabled": true
            }
         })JSON",
         {"dividend_yield", "dividend_stability", "industry_code", "market_cap"},
         factor::SourceTable::UNKNOWN,
         1},
        {QStringLiteral("technical_runtime_contract"),
         ...
         factor::SourceTable::UNKNOWN,
         250},
        {QStringLiteral("macro_runtime_contract"),
         ...
         {"close", "industrial_added_value_yoy", "industry_code", "market_cap"},
         factor::SourceTable::UNKNOWN,
         21},
        {QStringLiteral("industry_runtime_contract"),
         ...
         {"industry_concentration", "industry_code", "market_cap"},
         factor::SourceTable::UNKNOWN,
         20},
        {QStringLiteral("sentiment_runtime_contract"),
         ...
         {"sentiment_score", "industry_code", "market_cap"},
         factor::SourceTable::UNKNOWN,
         20},
        {QStringLiteral("custom_runtime_contract"),
         ...
         {"basis_rate", "hot_rank", "industry_code", "market_cap"},
         factor::SourceTable::UNKNOWN,
         1}
    };

    const auto toSet = [](const std::vector<std::string>& values) {
    ...
    for (const auto& testCase : cases) {
        ...
        const factor::DataRequirements requirements = factorInstance->getDataRequirements();
        const factor::BoundaryRules rules = factorInstance->getBoundaryRules();
        EXPECT_EQ(toSet(requirements.requiredFields), toSet(testCase.expectedRequiredFields));
        EXPECT_EQ(requirements.sourceTable, testCase.expectedSourceTable);
        EXPECT_EQ(rules.minDataPoints, testCase.expectedMinDataPoints);

        const auto serialized = factorInstance->toJson();
        ASSERT_TRUE(serialized.has("dataRequirements"));
        ASSERT_TRUE(serialized.has("boundaryRules"));

        const auto serializedRequirements = serialized.get("dataRequirements");
        ASSERT_TRUE(serializedRequirements.has("required"));
        EXPECT_EQ(toSet(jsonStringArray(serializedRequirements.get("required"))), toSet(requirements.requiredFields));
        ASSERT_TRUE(serializedRequirements.has("sourceTable"));
        EXPECT_EQ(serializedRequirements.get("sourceTable").asInt(), static_cast<int>(requirements.sourceTable));

        const auto serializedBoundaryRules = serialized.get("boundaryRules");
        ASSERT_TRUE(serializedBoundaryRules.has("minDataPoints"));
        EXPECT_EQ(serializedBoundaryRules.get("minDataPoints").asInt(), rules.minDataPoints);
    }
}
...
TEST(FactorBacktestRegressionTest, MomentumFactorCanApplyMinMaxStandardization)
{
    ...
    MomentumFactor::Params params;
    params.window = 1;
    params.standardization = factor::StandardizationMethod::MinMax;
    params.adjustPriceType = factor::AdjustPriceType::POST_ADJUST_FACTOR;
    factor.setParams(params);
...
TEST(FactorBacktestRegressionTest, CachedBarsKeepMarketCloseWhenFinancialRowsShareSameDate)
{
    QVariantMap marketRow;
    marketRow.insert(QStringLiteral("symbol"), QStringLiteral("600601.SH"));
    marketRow.insert(QStringLiteral("trade_date"), QStringLiteral("2023-05-17"));
    marketRow.insert(QStringLiteral("data_type"), QStringLiteral("market_bar"));
    marketRow.insert(QStringLiteral("close"), 16996.745426336);
    marketRow.insert(QStringLiteral("post_adjust_factor"), 6464.5419872);
    marketRow.insert(QStringLiteral("volume"), 12345.0);

    QVariantMap financialRow;
    financialRow.insert(QStringLiteral("symbol"), QStringLiteral("600601.SH"));
    financialRow.insert(QStringLiteral("trade_date"), QStringLiteral("2023-05-17"));
    financialRow.insert(QStringLiteral("data_type"), QStringLiteral("financial"));
    financialRow.insert(QStringLiteral("close"), 2.629999876);
    financialRow.insert(QStringLiteral("eps"), 0.42);
    financialRow.insert(QStringLiteral("report_date"), QStringLiteral("2023-03-31"));

    const QVariantList rows{marketRow, financialRow};
    const auto cachedBars = factor::cached_bars::buildCachedBarsFromRows(rows);

    ASSERT_EQ(cachedBars.size(), 1U);
    EXPECT_DOUBLE_EQ(cachedBars.front().close, 16996.745426336);

    const auto closeIt = cachedBars.front().numericFields.find("close");
    ASSERT_NE(closeIt, cachedBars.front().numericFields.end());
    EXPECT_DOUBLE_EQ(closeIt->second, 16996.745426336);

    const auto epsIt = cachedBars.front().numericFields.find("eps");
    ASSERT_NE(epsIt, cachedBars.front().numericFields.end());
    EXPECT_DOUBLE_EQ(epsIt->second, 0.42);

    const auto adjIt = cachedBars.front().numericFields.find("adj_factor");
    ASSERT_NE(adjIt, cachedBars.front().numericFields.end());
    EXPECT_DOUBLE_EQ(adjIt->second, 6464.5419872);
}
    ...
    factor::LowVolFactorTestAccess::loadConfig(
        ...
        jsonFacadeFromObjectForTest(QJsonObject{
            ...
            {QStringLiteral("calculation"), QJsonObject{
                ...
                {QStringLiteral("lookbackWindow"), 3},
...
TEST(FactorBacktestRegressionTest, QualityFactorCanUseLaggedEffectiveDateWithinLookbackWindow)
{
    ...
    ASSERT_TRUE(result.metadata.has("effectiveDate"));
    EXPECT_EQ(QString::fromStdString(result.metadata.get("effectiveDate").asString()), QStringLiteral("2024-01-12"));
    ASSERT_TRUE(result.metadata.has("lookbackWindow"));
    EXPECT_EQ(result.metadata.get("lookbackWindow").asInt(), 3);
...
TEST(FactorBacktestRegressionTest, BuildBacktestConfigEnablesDateParallelismForSingleFactorBatch)
{
    FactorBacktestController controller;
    controller.setDataSourceMode(QStringLiteral("cache"));
    controller.setSelectedDatasetId(48);
    ...
    FactorBacktestControllerTestAccess::primeSingleFactorCompletionState(
        controller,
        QVariantList{QStringLiteral("single_factor_instance")});

    const factor::BacktestConfig config = FactorBacktestControllerTestAccess::buildBacktestConfig(
        controller,
        QStringLiteral("single_factor_instance"),
        QStringLiteral("10组"),
        QStringLiteral("2024-01-01"),
        QStringLiteral("2024-01-31"));
...
TEST(FactorBacktestRegressionTest, BuildBacktestConfigUsesRequestedDefaultRuntimeValues)
{
    FactorBacktestController controller;
    controller.setDataSourceMode(QStringLiteral("cache"));
    controller.setSelectedDatasetId(48);
    controller.setSelectedDatasetBenchmarkMetadata(QVariantMap{{QStringLiteral("benchmarkSymbol"), QStringLiteral("000905.SH")}});
    ...
    EXPECT_EQ(config.forwardDays, risk::config::kDefaultForwardDays);
    EXPECT_EQ(config.rebalanceDays, risk::config::kDefaultRebalanceDays);
    EXPECT_DOUBLE_EQ(config.transactionCost, risk::config::kDefaultCommissionRate);
    EXPECT_DOUBLE_EQ(config.slippageRate, risk::config::kDefaultSlippageRate);
    EXPECT_DOUBLE_EQ(config.riskFreeRate, risk::config::kDefaultRiskFreeRate);
    EXPECT_EQ(QString::fromStdString(config.benchmarkSymbol), QStringLiteral("000905.SH"));
    EXPECT_DOUBLE_EQ(config.stopLossRate, risk::config::kDefaultStopLossPercent);
    EXPECT_DOUBLE_EQ(config.takeProfitRate, risk::config::kDefaultTakeProfitPercent);
    EXPECT_DOUBLE_EQ(config.maxDrawdownLimit, risk::config::kDefaultMaxDrawdownLimit);
    EXPECT_DOUBLE_EQ(config.maxDailyLoss, risk::config::kDefaultMaxDailyLoss);
    EXPECT_DOUBLE_EQ(config.maxPositionPercent, risk::config::kDefaultMaxPositionPercent);
    EXPECT_DOUBLE_EQ(config.maxTotalExposure, risk::config::kDefaultMaxTotalExposure);
}

TEST(FactorBacktestRegressionTest, BuildBacktestConfigDoesNotInterceptWhenRuntimeInstanceMissing)
{
    FactorBacktestController controller;
    controller.setDataSourceMode(QStringLiteral("cache"));
    controller.setSelectedDatasetId(48);

    const QString instanceId = QStringLiteral("momentum_factor_missing_runtime_instance");
    const char* configJson = R"JSON({
        "factorType": 1,
        "majorCategory": "动量因子",
        "calculation": {
    ...
    const auto instanceInfo = makeFactorInstanceInfo(
        ...
        QString::fromUtf8("动量因子"),
        configJson);
    const std::function<factor::FactorInstanceInfo(const QString&)> instanceInfoResolver =
        [instanceInfo](const QString&) -> factor::FactorInstanceInfo {
            return instanceInfo;
        };
    const std::function<std::shared_ptr<factor::BaseFactor>(const QString&)> factorInstanceResolver =
        [](const QString&) -> std::shared_ptr<factor::BaseFactor> {
            return {};
        };
    ...
    EXPECT_EQ(QString::fromStdString(config.instanceId), instanceId);
    EXPECT_EQ(config.datasetId, 48);
    EXPECT_EQ(QString::fromStdString(config.startDate), QStringLiteral("2024-01-01"));
    EXPECT_EQ(QString::fromStdString(config.endDate), QStringLiteral("2024-01-31"));
}

TEST(FactorBacktestRegressionTest, BuildBacktestConfigMergesDatabaseWarmupRowsIntoCachedBars)
{
    ...
    const auto tradeDateResult = database->executeQuery(
        QStringLiteral(
            "SELECT DATE_FORMAT(trade_date, '%Y-%m-%d') AS trade_date "
            "FROM (SELECT DISTINCT trade_date FROM daily_bar ORDER BY trade_date DESC LIMIT 12) recent_dates "
            "ORDER BY trade_date ASC"));
    if (tradeDateResult.rowCount() < 9) {
        cache.clearAllCache();
        GTEST_SKIP() << "insufficient daily_bar history for warmup regression";
    }

    QStringList tradeDates;
    tradeDates.reserve(static_cast<int>(tradeDateResult.rowCount()));
    for (const auto& row : tradeDateResult.getRows()) {
        const QString tradeDate = row.getString(QStringLiteral("trade_date")).trimmed();
        if (!tradeDate.isEmpty()) {
            tradeDates.append(tradeDate);
        }
    }
    if (tradeDates.size() < 9) {
        cache.clearAllCache();
        GTEST_SKIP() << "insufficient normalized trade dates for warmup regression";
    }

    const qsizetype anchorIndex = tradeDates.size() - 4;
    if (anchorIndex < 5) {
        cache.clearAllCache();
        GTEST_SKIP() << "insufficient leading trade dates for warmup regression";
    }

    const QString startDate = tradeDates.at(anchorIndex);
    const QString endDate = tradeDates.back();
    const QString expectedWarmupStartDate = tradeDates.at(anchorIndex - 5);
    const int requiredDateCount = static_cast<int>(tradeDates.size() - (anchorIndex - 5));

    const auto symbolResult = database->executeQuery(
        QStringLiteral(
            "SELECT symbol "
            "FROM daily_bar "
            "WHERE trade_date BETWEEN :history_start AND :end_date "
            "GROUP BY symbol "
            "HAVING COUNT(DISTINCT trade_date) >= :required_dates "
            "ORDER BY symbol LIMIT 1"),
        {{QStringLiteral(":history_start"), expectedWarmupStartDate},
         {QStringLiteral(":end_date"), endDate},
         {QStringLiteral(":required_dates"), requiredDateCount}});
    if (symbolResult.isEmpty()) {
        cache.clearAllCache();
        GTEST_SKIP() << "no symbol with sufficient daily_bar coverage for warmup regression";
    }
    const QString symbol = symbolResult.getRow(0).getString(QStringLiteral("symbol")).trimmed();
    ASSERT_FALSE(symbol.isEmpty());

    const auto datasetRowsResult = database->executeQuery(
        QStringLiteral(
            "SELECT symbol, DATE_FORMAT(trade_date, '%Y-%m-%d') AS trade_date, close, post_adjust_factor "
            "FROM daily_bar "
        ...
        {{QStringLiteral(":symbol"), symbol},
         ...
         {QStringLiteral(":end_date"), endDate}});
    ASSERT_FALSE(datasetRowsResult.isEmpty());

    QVariantList baseRows;
    baseRows.reserve(static_cast<int>(datasetRowsResult.rowCount()));
    for (const auto& row : datasetRowsResult.getRows()) {
        QVariantMap rowMap;
        for (const auto& entry : row.getValues()) {
            rowMap.insert(entry.first, entry.second);
        }
        baseRows.append(rowMap);
    }
    ASSERT_FALSE(baseRows.isEmpty());
    ...
    const QString instanceId = QStringLiteral("momentum_warmup_db_instance");
    const auto instanceInfo = makeFactorInstanceInfo(
        instanceId,
        QString::fromUtf8("动量因子"),
        R"JSON({
    ...
    ASSERT_TRUE(config.preparedArrowData || !config.cachedBars.empty());
    if (config.preparedArrowData) {
        EXPECT_GT(config.preparedArrowData->rowCount(), static_cast<int>(baseRows.size()));
    } else {
        EXPECT_GT(config.cachedBars.size(), static_cast<size_t>(baseRows.size()));
    }

    std::vector<std::string> mergedTradeDates;
    if (config.preparedArrowData) {
        const auto mergedSeries = config.preparedArrowData->getSeries(
            symbol.toStdString(),
            expectedWarmupStartDate.toStdString(),
            endDate.toStdString(),
            "close");
        mergedTradeDates.reserve(mergedSeries.size());
        for (const auto& point : mergedSeries) {
            mergedTradeDates.push_back(point.date);
        }
    } else {
        mergedTradeDates = factor::cached_bars::extractTradeDates(config.cachedBars, std::string(), std::string());
    }
    ASSERT_FALSE(mergedTradeDates.empty());
    EXPECT_EQ(QString::fromStdString(mergedTradeDates.front()), expectedWarmupStartDate);
    EXPECT_EQ(QString::fromStdString(mergedTradeDates.back()), endDate);
    EXPECT_LT(QString::fromStdString(mergedTradeDates.front()), startDate);

    cache.clearAllCache();
}

TEST(FactorBacktestRegressionTest, BuildBacktestConfigMergesDatabaseWarmupRowsWithoutRuntimeInstance)
{
    auto database = astock::database::DatabaseConnectionManager::instance().getDatabase();
    if (!database) {
        GTEST_SKIP() << "database unavailable";
    ...
    const QString instanceId = QStringLiteral("momentum_warmup_missing_runtime_instance");
    const auto instanceInfo = makeFactorInstanceInfo(
        instanceId,
        QString::fromUtf8("动量因子"),
        R"JSON({
    ...
    factor::ValueFactorTestAccess::loadConfig(
        ...
        jsonFacadeFromObjectForTest(QJsonObject{
            ...
            {QStringLiteral("calculation"), QJsonObject{
                ...
                {QStringLiteral("standardization"), static_cast<int>(factor::StandardizationMethod::Percentile)}
            }}
        }));
...
{
    constexpr const char* kInstanceId = "fixed_value_bp_runtime_params";
    const char* configJson = R"JSON({
        "factorType": "value",
        "majorCategory": "价值因子",
        "calculation": {
    ...
    const auto fullConfig = foundation::json::JsonFacade::parse(configJson);
    ...
    factor::ValueFactorTestAccess::loadConfig(*factorInstance, fullConfig);

    auto instanceManager = std::make_shared<factor::FactorInstanceManager>(nullptr, nullptr);
    factor::FactorInstanceManagerTestAccess::seedInstance(
        *instanceManager,
        QString::fromUtf8(kInstanceId),
        makeFactorInstanceInfo(QString::fromUtf8(kInstanceId), QString::fromUtf8("价值因子"), configJson),
        factorInstance);
    auto createdInstance = instanceManager->createInstance(kInstanceId);
    ASSERT_NE(createdInstance, nullptr);

    const QString latestDate = QStringLiteral("2024-01-15");
    factor::CalculationContext context;
    ...
    const auto result = createdInstance->calculate(context);
    ASSERT_NE(factorInstance, nullptr);
    ...
    EXPECT_TRUE(result.metadata.get("laggedEnabled").asBool());
    ASSERT_TRUE(result.metadata.has("lookbackWindow"));
    EXPECT_EQ(result.metadata.get("lookbackWindow").asInt(), 252);
    ASSERT_TRUE(result.metadata.has("standardization"));
...
TEST(FactorBacktestRegressionTest, RealMomentumFactorInstanceReplayUsesConfiguredRuntimeParameters)
{
    ...
    ASSERT_TRUE(result.metadata.has("lookbackWindow"));
    EXPECT_EQ(result.metadata.get("lookbackWindow").asInt(), expectedConfigurableLookbackWindow(calculation));
    ASSERT_TRUE(result.metadata.has("standardization"));
...
TEST(FactorBacktestRegressionTest, RealQualityRoeCachedBacktestReplayReportsCurrentMonotonicity)
{
    auto database = astock::database::DatabaseConnectionManager::instance().getDatabase();
    if (!database) {
        GTEST_SKIP() << "database unavailable";
    ...
    auto& cache = DataServiceCache::getInstance();
    ASSERT_TRUE(cache.initializeCache());
    const int datasetId = 67;
    const QVariantList datasetRows = cache.getDataSetById(datasetId);
    if (datasetRows.isEmpty()) {
        GTEST_SKIP() << "dataset 67 unavailable in local cache";
    }

    constexpr const char* kInstanceId = "__________252__90______1776674045225_fa0002a9";
    auto dataChecker = std::make_shared<factor::DataAvailabilityChecker>(database);
    auto instanceManager = std::make_shared<factor::FactorInstanceManager>(database, dataChecker);
    const auto instanceInfo = instanceManager->getInstanceInfo(kInstanceId);
    if (instanceInfo.instanceId.empty()) {
        GTEST_SKIP() << "target quality instance unavailable in local database";
    }

    FactorBacktestController controller;
    controller.setDataSourceMode(QStringLiteral("cache"));
    controller.setSelectedDatasetId(datasetId);
    controller.setSelectedDatasetBenchmarkMetadata(QVariantMap{
        {QStringLiteral("indexSymbol"), QStringLiteral("000300.SH")},
        {QStringLiteral("benchmarkSymbol"), QStringLiteral("000300.SH")}
    });
    controller.setBacktestRuntimeParams(QVariantMap{
        {QStringLiteral("marketEnvironmentProfile"), factor::marketEnvironmentProfileIndex(factor::MarketEnvironmentProfile::CN_A_SHARE)},
        {QStringLiteral("forwardDays"), 45},
        {QStringLiteral("rebalanceDays"), 15},
        {QStringLiteral("commissionRate"), 0.0003},
        {QStringLiteral("slippageRate"), 0.001},
        {QStringLiteral("riskFreeRate"), 0.0002},
        {QStringLiteral("benchmarkSymbol"), QStringLiteral("000905.SH")}
    });

    auto cacheManager = std::make_shared<factor::FactorCacheManager>();
    auto threadPool = std::make_shared<foundation::thread::ThreadPoolExecutor>(1);
    FactorBacktestControllerTestAccess::primeAsyncBacktestRuntime(controller, cacheManager, threadPool, instanceManager);
    FactorBacktestControllerTestAccess::setDatabase(controller, database);

    BacktestConfig config;
    try {
        config = FactorBacktestControllerTestAccess::buildBacktestConfig(
            controller,
            QString::fromUtf8(kInstanceId),
            QStringLiteral("5组"),
            QStringLiteral("2020-01-02"),
            QStringLiteral("2026-05-22"));
    } catch (const std::runtime_error& e) {
        GTEST_SKIP() << e.what();
    }

    auto formatGroupAnnualReturns = [](const std::vector<double>& values) {
        QStringList texts;
        texts.reserve(static_cast<qsizetype>(values.size()));
        for (double value : values) {
            texts.append(QString::number(value, 'g', 17));
        }
        return texts.join(',');
    };
    ...
    ASSERT_EQ(result.factorMetrics.groupAnnualReturns.size(), 5U);
    ASSERT_TRUE(std::isfinite(result.factorMetrics.monotonicityScore));
    ASSERT_FALSE(config.allowedStockCodesByDate.empty());
        const auto startDateAllowedIt = config.allowedStockCodesByDate.find("2020-01-02");
    qDebug() << "RealQualityRoeCachedBacktestReplayReportsCurrentMonotonicity"
             << "instanceId=" << QString::fromUtf8(kInstanceId)
             << "datasetId=" << datasetId
             << "allowedStockCodesByDateCount=" << static_cast<int>(config.allowedStockCodesByDate.size())
              << "startDateAllowedPresent=" << (startDateAllowedIt != config.allowedStockCodesByDate.end())
              << "startDateAllowedSymbolCount="
              << static_cast<int>(startDateAllowedIt != config.allowedStockCodesByDate.end()
                  ? startDateAllowedIt->second.size()
                  : 0)
             << "monotonicity=" << result.factorMetrics.monotonicityScore
             << "groupAnnualReturns=" << formatGroupAnnualReturns(result.factorMetrics.groupAnnualReturns);
}

TEST(FactorBacktestRegressionTest, BuildBacktestConfigRejectsDynamicIndexHistoryGapAtStartDate)
{
    auto database = astock::database::DatabaseConnectionManager::instance().getDatabase();
    if (!database) {
        GTEST_SKIP() << "database unavailable";
    ...
    const QString instanceId = QStringLiteral("size_dynamic_index_gap_guard");
    ...
    instanceInfo.instanceName = "Size Dynamic Index Gap Guard";
    instanceInfo.description = "Size Dynamic Index Gap Guard";
    ...
    const QString indexSymbol = QStringLiteral("TGI_%1")
        .arg(QString::number(qHash(QUuid::createUuid().toString(QUuid::WithoutBraces)), 16).right(8));
    database->executeUpdate(
        QStringLiteral("DELETE FROM index_constituents WHERE index_symbol = :index_symbol"),
        {{QStringLiteral(":index_symbol"), indexSymbol}});

    database->executeUpdate(
        QStringLiteral(
            "INSERT INTO index_constituents (index_symbol, constituent_symbol, start_date, end_date) "
            "VALUES (:index_symbol, :constituent_symbol, :start_date, :end_date)"),
        {{QStringLiteral(":index_symbol"), indexSymbol},
         {QStringLiteral(":constituent_symbol"), QStringLiteral("000001.SZ")},
         {QStringLiteral(":start_date"), QStringLiteral("2020-01-03")},
         {QStringLiteral(":end_date"), QStringLiteral("2020-01-31")}});

    FactorBacktestController controller;
    controller.setDataSourceMode(QStringLiteral("cache"));
    controller.setSelectedDatasetId(1);
    controller.setSelectedDatasetBenchmarkMetadata(QVariantMap{{QStringLiteral("indexSymbol"), indexSymbol}});
    FactorBacktestControllerTestAccess::configureSupportMapOverrides(controller, instanceInfo, factorInstance);
    FactorBacktestControllerTestAccess::setDatabase(controller, database);
    ...
    try {
        Q_UNUSED(FactorBacktestControllerTestAccess::buildBacktestConfig(
            ...
            QStringLiteral("5组"),
            QStringLiteral("2020-01-02"),
            QStringLiteral("2020-01-05")));
        FAIL() << "expected runtime_error for missing dynamic index coverage";
    } catch (const std::runtime_error& e) {
        const QString errorText = QString::fromUtf8(e.what());
        EXPECT_TRUE(errorText.contains(QStringLiteral("指数历史成分股未覆盖回测日期 2020-01-02")));
        EXPECT_TRUE(errorText.contains(QStringLiteral("禁止回退到裸缓存集")));
    }
...
TEST(FactorBacktestRegressionTest, BuildBacktestConfigUsesAnnouncementWindowForDynamicIndexConstituents)
{
    auto database = astock::database::DatabaseConnectionManager::instance().getDatabase();
    if (!database) {
        GTEST_SKIP() << "database unavailable";
    ...
    bool hasAnnouncementDateColumn = false;
    try {
        const auto columnResult = database->executeQuery(
            QStringLiteral(
                "SELECT 1 "
                ...
                "LIMIT 1"),
            {});
        hasAnnouncementDateColumn = columnResult.rowCount() > 0;
    } catch (const std::exception&) {
        hasAnnouncementDateColumn = false;
    }

    if (!hasAnnouncementDateColumn) {
        GTEST_SKIP() << "index_constituents.announcement_date unavailable";
    }

    const QString instanceId = QStringLiteral("size_dynamic_index_announcement_window");
    factor::FactorInstanceInfo instanceInfo;
    instanceInfo.instanceId = instanceId.toStdString();
    instanceInfo.factorType = factor::FactorType::SIZE;
    instanceInfo.instanceName = "Size Dynamic Index Announcement Window";
    instanceInfo.description = "Size Dynamic Index Announcement Window";
    instanceInfo.isAvailable = true;
    instanceInfo.config = parseRuntimeCompatibleTestConfig(R"JSON({
        "factorType": "size",
        "calculation": {
    ...
    const QString indexSymbol = QStringLiteral("TAW_%1")
        .arg(QString::number(qHash(QUuid::createUuid().toString(QUuid::WithoutBraces)), 16).right(8));
    database->executeUpdate(
        QStringLiteral("DELETE FROM index_constituents WHERE index_symbol = :index_symbol"),
        {{QStringLiteral(":index_symbol"), indexSymbol}});
    ...
    database->executeUpdate(
        QStringLiteral(
            ...
            "(index_symbol, constituent_symbol, announcement_date, start_date, end_date) "
            "VALUES (:index_symbol, :constituent_symbol, :announcement_date, :start_date, :end_date)"),
        {{QStringLiteral(":index_symbol"), indexSymbol},
         {QStringLiteral(":constituent_symbol"), QStringLiteral("000001.SZ")},
         {QStringLiteral(":announcement_date"), QStringLiteral("2020-01-02")},
         {QStringLiteral(":start_date"), QStringLiteral("2020-01-06")},
         {QStringLiteral(":end_date"), QStringLiteral("2020-01-31")}});
    ...
    factor::BacktestConfig config;
    ASSERT_NO_THROW(config = FactorBacktestControllerTestAccess::buildBacktestConfig(
        controller,
        instanceId,
        QStringLiteral("5组"),
        QStringLiteral("2020-01-03"),
        QStringLiteral("2020-01-06")));

    const auto allowedIt = config.allowedStockCodesByDate.find("2020-01-03");
    ASSERT_NE(allowedIt, config.allowedStockCodesByDate.end());
    EXPECT_EQ(allowedIt->second, (std::vector<std::string>{"000001.SZ"}));
...
{
    constexpr const char* kInstanceId = "fixed_lowvol_runtime_params";
    const char* configJson = R"JSON({
        "factorType": "low_volatility",
        "majorCategory": "低波因子",
        "calculation": {
    ...
    factor::LowVolFactorTestAccess::loadConfig(*factorInstance, fullConfig);

    auto instanceManager = std::make_shared<factor::FactorInstanceManager>(nullptr, nullptr);
    factor::FactorInstanceManagerTestAccess::seedInstance(
        *instanceManager,
        QString::fromUtf8(kInstanceId),
        makeFactorInstanceInfo(QString::fromUtf8(kInstanceId), QString::fromUtf8("低波因子"), configJson),
        factorInstance);
    auto createdInstance = instanceManager->createInstance(kInstanceId);
    ASSERT_NE(createdInstance, nullptr);
    ...
    assignHistoricalSeries(fieldSeries, "market_cap", "AAA", buildLinearHistoricalSeries(latestDate, 80, 120.0, 1.0));
    assignHistoricalSeries(fieldSeries, "market_cap", "BBB", buildLinearHistoricalSeries(latestDate, 80, 180.0, 1.0));
    assignHistoricalSeries(fieldSeries, "market_cap", "CCC", buildLinearHistoricalSeries(latestDate, 80, 260.0, 1.0));
    assignHistoricalSeries(fieldSeries, "industry_code", "AAA", buildLinearHistoricalSeries(latestDate, 80, 10.0, 0.0));
    assignHistoricalSeries(fieldSeries, "industry_code", "BBB", buildLinearHistoricalSeries(latestDate, 80, 10.0, 0.0));
    assignHistoricalSeries(fieldSeries, "industry_code", "CCC", buildLinearHistoricalSeries(latestDate, 80, 20.0, 0.0));
    context.historicalView = std::make_shared<DatedMultiFieldFactorDataProvider>(std::move(fieldSeries));
...
TEST(FactorBacktestRegressionTest, LowVolReplayOnInitialCacheDateRequiresDatabaseWarmupRows)
{
    auto database = astock::database::DatabaseConnectionManager::instance().getDatabase();
    if (!database) {
        GTEST_SKIP() << "database unavailable";
    ...
    if (tradeDateResult.rowCount() < 9) {
        cache.clearAllCache();
        GTEST_SKIP() << "insufficient daily_bar history for lowvol warmup regression";
    }
    ...
    if (tradeDates.size() < 9) {
        cache.clearAllCache();
        GTEST_SKIP() << "insufficient normalized trade dates for lowvol warmup regression";
    }

    const qsizetype anchorIndex = tradeDates.size() - 4;
    if (anchorIndex < 5) {
        cache.clearAllCache();
        GTEST_SKIP() << "insufficient leading trade dates for lowvol warmup regression";
    }
    ...
    const auto symbolResult = database->executeQuery(
        ...
        {{QStringLiteral(":history_start"), expectedWarmupStartDate},
         {QStringLiteral(":end_date"), endDate},
         {QStringLiteral(":required_dates"), requiredDateCount}});
    if (symbolResult.rowCount() < 3) {
        cache.clearAllCache();
        GTEST_SKIP() << "insufficient symbols with contiguous daily_bar coverage for lowvol warmup regression";
    }

    QStringList symbols;
    symbols.reserve(static_cast<int>(symbolResult.rowCount()));
    for (const auto& row : symbolResult.getRows()) {
    ...
    if (symbols.size() < 3) {
        cache.clearAllCache();
        GTEST_SKIP() << "insufficient normalized symbols for lowvol warmup regression";
    }

    QStringList escapedSymbols;
    escapedSymbols.reserve(symbols.size());
    for (QString symbol : symbols) {
        symbol.replace(QStringLiteral("'"), QStringLiteral("''"));
        escapedSymbols.append(QStringLiteral("'%1'").arg(symbol));
    }

    const auto datasetRowsResult = database->executeQuery(
        QStringLiteral(
            "SELECT symbol, DATE_FORMAT(trade_date, '%Y-%m-%d') AS trade_date, close "
            "FROM daily_bar "
            "WHERE symbol IN (%1) AND trade_date BETWEEN :start_date AND :end_date "
            "ORDER BY symbol ASC, trade_date ASC")
            .arg(escapedSymbols.join(QStringLiteral(", "))),
        {{QStringLiteral(":start_date"), startDate},
         {QStringLiteral(":end_date"), endDate}});
    ASSERT_FALSE(datasetRowsResult.isEmpty());
    ...
    const QString instanceId = QStringLiteral("lowvol_warmup_replay_instance");
    const char* configJson = R"JSON({
        "factorType": "low_volatility",
        "majorCategory": "低波因子",
        "calculation": {
    ...
    const auto makeHistoricalView = [](const std::shared_ptr<factor::ArrowMarketData>& data)
        -> std::shared_ptr<factor::HistoricalView> {
        class ArrowHistoricalViewAdapter final : public factor::HistoricalView {
        public:
            explicit ArrowHistoricalViewAdapter(std::shared_ptr<factor::ArrowMarketData> data)
                : data_(std::move(data))
            {
            }
            ...
            bool hasField(const std::string& field) const override
            {
                return data_ && static_cast<bool>(data_->getColumn(field));
            }
                                           ...
                                           const std::string& field) const override
            {
                ...
                const auto crossSection = data_->getCrossSection(date, field, {symbol});
                const auto it = crossSection.find(symbol);
                if (it == crossSection.end()) {
                    return std::nullopt;
                }
                return it->second;
            }
            ...
            std::vector<factor::HistoricalDataPoint> getSeries(const std::string& symbol,
                                                               ...
                                                               const std::string& field) const override
            {
                return data_ ? data_->getSeries(symbol, startDate, endDate, field) : std::vector<factor::HistoricalDataPoint>{};
            }

            std::vector<std::string> getAvailableSymbols(const std::string& date) const override
            {
                return data_ ? data_->getAvailableSymbols(date) : std::vector<std::string>{};
            }
            ...
            std::unordered_map<std::string, double> getCrossSection(
                ...
                const std::vector<std::string>& symbols = {}) const override
            {
                return data_ ? data_->getCrossSection(date, field, symbols)
            ...
            std::unordered_map<std::string, std::unordered_map<std::string, double>> getBatchCrossSections(
                ...
                const std::vector<std::string>& fields) const override
            {
                return data_ ? data_->getBatchCrossSections(date, symbols, fields)
                             : std::unordered_map<std::string, std::unordered_map<std::string, double>>{};
            }
            ...
            std::unordered_map<std::string, std::unordered_map<std::string, std::vector<double>>> getBatchTimeSeries(
                ...
                const std::vector<std::string>& fields) const override
            {
                return data_ ? data_->getBatchTimeSeries(symbols, startDate, endDate, fields)
        ...
        return std::make_shared<ArrowHistoricalViewAdapter>(data);
    };

    const auto rawCachedBars = factor::cached_bars::buildCachedBarsFromRows(baseRows);
    const auto rawHistoricalView = makeHistoricalView(factor::ArrowMarketData::fromCachedBars(rawCachedBars));
    ASSERT_NE(rawHistoricalView, nullptr);

    factor::CalculationContext rawContext;
    rawContext.date = startDate.toStdString();
    rawContext.symbols = rawHistoricalView->getAvailableSymbols(startDate.toStdString());
    ...
    ASSERT_TRUE(config.preparedArrowData || !config.cachedBars.empty());
    if (config.preparedArrowData) {
        EXPECT_GT(config.preparedArrowData->rowCount(), static_cast<int>(rawCachedBars.size()));
    } else {
        EXPECT_GT(config.cachedBars.size(), rawCachedBars.size());
    }
    std::vector<std::string> mergedTradeDates;
    if (config.preparedArrowData) {
        const auto mergedSeries = config.preparedArrowData->getSeries(
            symbols.front().toStdString(),
            expectedWarmupStartDate.toStdString(),
            endDate.toStdString(),
            "close");
        mergedTradeDates.reserve(mergedSeries.size());
    ...
    const auto warmedHistoricalView = config.preparedArrowData
        ? makeHistoricalView(config.preparedArrowData)
        : makeHistoricalView(factor::ArrowMarketData::fromCachedBars(config.cachedBars));
    ASSERT_NE(warmedHistoricalView, nullptr);

    factor::CalculationContext warmedContext;
    warmedContext.date = startDate.toStdString();
    warmedContext.symbols = warmedHistoricalView->getAvailableSymbols(startDate.toStdString());
    warmedContext.historicalView = warmedHistoricalView;

    const auto warmedResult = factorInstance->calculate(warmedContext);
    ASSERT_TRUE(warmedResult.dataStatus.isValid()) << warmedResult.dataStatus.message;
    EXPECT_FALSE(warmedResult.values.empty());
    ASSERT_TRUE(warmedResult.metadata.has("effectiveDate"));
    EXPECT_EQ(QString::fromStdString(warmedResult.metadata.get("effectiveDate").asString()), startDate);
...
{
    constexpr const char* kInstanceId = "fixed_liquidity_runtime_params";
    const char* configJson = R"JSON({
        "factorType": "liquidity",
        "majorCategory": "流动性因子",
        "calculation": {
    ...
    factor::FactorInstanceManagerTestAccess::seedInstance(
        *instanceManager,
        QString::fromUtf8(kInstanceId),
        makeFactorInstanceInfo(QString::fromUtf8(kInstanceId), QString::fromUtf8("流动性因子"), configJson),
        factorInstance);
    auto createdInstance = instanceManager->createInstance(kInstanceId);
    ASSERT_NE(createdInstance, nullptr);
...
TEST(FactorBacktestRegressionTest, RealGrowthFactorInstanceReplayUsesConfiguredRuntimeParameters)
{
    ...
    const auto replayHandle = ensureReplayInstanceByCategory(
        ...
        QJsonObject{
            ...
            {QStringLiteral("calculation"), QJsonObject{{QStringLiteral("growthMetrics"), QJsonArray{QStringLiteral("revenue_growth")}}, {QStringLiteral("growthWeights"), QJsonArray{1.0}}, {QStringLiteral("lookbackWindow"), 252}}},
...
TEST(FactorBacktestRegressionTest, RealGrowthInstanceDataset48ExecutesWithoutAllDatesEmptyFailure)
{
    auto database = astock::database::DatabaseConnectionManager::instance().getDatabase();
    if (!database) {
        GTEST_SKIP() << "database unavailable";
    ...
    auto& cache = DataServiceCache::getInstance();
    if (!cache.initializeCache()) {
        GTEST_SKIP() << "cache unavailable";
    }

    const QString localAppData = qEnvironmentVariable("LOCALAPPDATA");
    if (localAppData.trimmed().isEmpty()) {
        GTEST_SKIP() << "LOCALAPPDATA unavailable";
    }

    const QString datasetRoot = QDir(localAppData).filePath(QStringLiteral("astockquantapp-exe/datasets"));
    QFile dataFile(QDir(datasetRoot).filePath(QStringLiteral("dataset_48_data.json")));
    if (!dataFile.exists() || !dataFile.open(QIODevice::ReadOnly)) {
        GTEST_SKIP() << "dataset 48 data file unavailable";
    }

    QJsonParseError dataParseError;
    const QJsonDocument dataDocument = QJsonDocument::fromJson(dataFile.readAll(), &dataParseError);
    if (dataParseError.error != QJsonParseError::NoError || !dataDocument.isArray()) {
        GTEST_SKIP() << "dataset 48 data file invalid";
    }

    const QVariantList datasetRows = dataDocument.array().toVariantList();
    if (datasetRows.isEmpty()) {
        GTEST_SKIP() << "dataset 48 data file empty";
    }

    QFile infoFile(QDir(datasetRoot).filePath(QStringLiteral("dataset_48_info.json")));
    QJsonObject infoObject;
    if (infoFile.exists() && infoFile.open(QIODevice::ReadOnly)) {
        QJsonParseError infoParseError;
        const QJsonDocument infoDocument = QJsonDocument::fromJson(infoFile.readAll(), &infoParseError);
        if (infoParseError.error == QJsonParseError::NoError && infoDocument.isObject()) {
            infoObject = infoDocument.object();
        }
    }

    QStringList availableFields;
    const QJsonArray availableFieldsArray = infoObject.value(QStringLiteral("availableFields")).toArray();
    for (const QJsonValue& fieldValue : availableFieldsArray) {
    ...
    const QJsonArray stockCodesArray = infoObject.value(QStringLiteral("stockCodes")).toArray();
    for (const QJsonValue& stockValue : stockCodesArray) {
        const QString stockCode = stockValue.toString().trimmed();
        if (!stockCode.isEmpty()) {
            stockCodes.append(stockCode);
        }
    }

    const QString startDate = infoObject.value(QStringLiteral("startDate")).toString().trimmed();
    const QString endDate = infoObject.value(QStringLiteral("endDate")).toString().trimmed();

    DataServiceCacheTestAccess::resetInMemoryIndex(cache);
    const int importedDatasetId = storeSupportMapDataset(
        datasetRows,
        availableFields,
        stockCodes,
        startDate,
        endDate);
    ASSERT_GT(importedDatasetId, 0);

    bool hasDisclosureOnlyFinancialRow = false;
    for (const QVariant& rowValue : datasetRows) {
        const QVariantMap row = rowValue.toMap();
        if (row.value(QStringLiteral("trade_date")).toString().trimmed().isEmpty()
            && !row.value(QStringLiteral("disclosure_date")).toString().trimmed().isEmpty()
            && row.value(QStringLiteral("total_revenue")).isValid()) {
            hasDisclosureOnlyFinancialRow = true;
            break;
        }
    }
    if (!hasDisclosureOnlyFinancialRow) {
        GTEST_SKIP() << "dataset 48 has no disclosure-only financial rows";
    }

    const QString instanceId = QStringLiteral("___________________252_1777305617877_33e3b8ca");
    const auto configResult = database->executeQuery(
        QStringLiteral(
            "SELECT fi.instance_name, f.major_category, CAST(fi.full_config AS CHAR) AS full_config "
            "FROM factor_instance fi "
            "LEFT JOIN factors f ON fi.factor_id = f.factor_id "
            "WHERE fi.instance_id = :instance_id LIMIT 1"),
        {{QStringLiteral(":instance_id"), instanceId}});
    if (configResult.isEmpty()) {
        GTEST_SKIP() << "growth instance unavailable in database";
    }

    const auto& configRow = configResult.getRow(0);
    const QString majorCategory = configRow.getString(QStringLiteral("major_category")).trimmed().isEmpty()
        ? QStringLiteral("成长因子")
        : configRow.getString(QStringLiteral("major_category"));
    const QByteArray fullConfigJson = configRow.getString(QStringLiteral("full_config")).toUtf8();

    auto factorInstance = makeConfiguredFactor(fullConfigJson.constData());
    ASSERT_NE(factorInstance, nullptr);
    ...
    factor::FactorInstanceManagerTestAccess::seedInstance(
        *instanceManager,
        instanceId,
        makeFactorInstanceInfo(instanceId, majorCategory, fullConfigJson.constData()),
        factorInstance);

    BacktestConfig config = makeCachedBacktestConfig(instanceId.toStdString());
    config.datasetId = importedDatasetId;
    config.startDate = "2023-01-03";
    config.endDate = "2026-05-13";
    config.forwardDays = 1;
    ...
    EXPECT_NE(result.status, std::string("FAILED")) << result.errorMessage;
    EXPECT_GT(result.dataCoverage, 0.0) << result.errorMessage;
    EXPECT_FALSE(QString::fromStdString(result.errorMessage).contains(QStringLiteral("因子在全部交易日都未产出有效值")))
        << result.errorMessage;
...
TEST(FactorBacktestRegressionTest, ConfigurableLiquidityFactorCanUseLaggedEffectiveDateWithinLookbackWindow)
{
    factor::ConfigurableFactorBase factor;
    factor::ConfigurableFactorTestAccess::loadConfig(factor, foundation::json::JsonFacade::parse(R"JSON({
        "factorType": "liquidity",
    ...
    context.historicalView = std::make_shared<DatedMultiFieldFactorDataProvider>(
        DatedMultiFieldFactorDataProvider::FieldSeriesMap{
            {"turnover_rate", {
                {"AAA", {{"2024-01-11", 1.0}, {"2024-01-12", 1.0}, {"2024-01-15", 10.0}}},
                {"BBB", {{"2024-01-11", 4.0}, {"2024-01-12", 4.0}, {"2024-01-15", 2.0}}}
            }}
        });
...
TEST(FactorBacktestRegressionTest, ConfigurableLiquidityFactorCanApplyMinMaxStandardization)
{
    factor::ConfigurableFactorBase factor;
    factor::ConfigurableFactorTestAccess::loadConfig(factor, foundation::json::JsonFacade::parse(R"JSON({
        "factorType": "liquidity",
    ...
    context.historicalView = std::make_shared<DatedMultiFieldFactorDataProvider>(
        DatedMultiFieldFactorDataProvider::FieldSeriesMap{
            {"turnover_rate", {
                {"AAA", {{"2024-01-11", 1.0}, {"2024-01-12", 1.0}, {"2024-01-15", 1.0}}},
                {"BBB", {{"2024-01-11", 3.0}, {"2024-01-12", 3.0}, {"2024-01-15", 3.0}}},
                {"CCC", {{"2024-01-11", 5.0}, {"2024-01-12", 5.0}, {"2024-01-15", 5.0}}}
            }}
        });
...
TEST(FactorBacktestRegressionTest, ConfigurableLiquidityFactorAppliesHistoricalViewIndustrySizeNeutralization)
{
    factor::ConfigurableFactorBase factor;
    factor::ConfigurableFactorTestAccess::loadConfig(factor, foundation::json::JsonFacade::parse(R"JSON({
        "factorType": "liquidity",
    ...
    context.historicalView = std::make_shared<DatedMultiFieldFactorDataProvider>(
        DatedMultiFieldFactorDataProvider::FieldSeriesMap{
            {"turnover_rate", {
                {"AAA", {{"2024-01-12", 2.0}, {"2024-01-15", 2.0}}},
                {"BBB", {{"2024-01-12", 4.0}, {"2024-01-15", 4.0}}},
                {"CCC", {{"2024-01-12", 6.0}, {"2024-01-15", 6.0}}},
                {"DDD", {{"2024-01-12", 8.0}, {"2024-01-15", 8.0}}}
            }},
            {"market_cap", {
                {"AAA", {{"2024-01-15", 100.0}}},
                {"BBB", {{"2024-01-15", 200.0}}},
    ...
    ASSERT_TRUE(result.metadata.has("neutralizationEnabled"));
    EXPECT_TRUE(result.metadata.get("neutralizationEnabled").asBool());
    ASSERT_TRUE(result.metadata.has("neutralizationMode"));
    EXPECT_FALSE(QString::fromStdString(scalarTextForTest(result.metadata.get("neutralizationMode"))).isEmpty());
}
...
TEST(FactorBacktestRegressionTest, RealMacroFactorInstanceReplayUsesConfiguredRuntimeParameters)
{
    ...
    ASSERT_TRUE(result.metadata.has("lookbackWindow"));
    EXPECT_EQ(result.metadata.get("lookbackWindow").asInt(), calculation.get("lookbackWindow").asInt());
    ASSERT_TRUE(result.metadata.has("laggedEnabled"));
    ...
    const auto replayHandle = ensureReplayInstanceByCategory(
        ...
        QJsonObject{
            ...
            {QStringLiteral("calculation"), QJsonObject{
                ...
                {QStringLiteral("lookbackWindow"), 5},
                {QStringLiteral("laggedEnabled"), true},
...
TEST(FactorBacktestRegressionTest, ExecutorFutureReturnUsesExactMarketTradingDayNotNextObservedBar)
{
    const QString instanceId = QStringLiteral("size_future_return_exact_market_day");
    factor::FactorInstanceInfo instanceInfo;
    instanceInfo.instanceId = instanceId.toStdString();
    instanceInfo.factorType = factor::FactorType::SIZE;
    instanceInfo.instanceName = "Size Future Return Exact Market Day";
    instanceInfo.description = "Size Future Return Exact Market Day";
    instanceInfo.isAvailable = true;
    instanceInfo.config = parseRuntimeCompatibleTestConfig(R"JSON({
        "factorType": "size",
        "calculation": {
    ...
    BacktestConfig config = makeCachedBacktestConfig(instanceId.toStdString());
    config.startDate = "2024-01-02";
    config.endDate = "2024-01-02";
    config.forwardDays = 1;
    config.numGroups = 2;

    config.cachedBars = {
        {"AAA", "2024-01-02", 10.0, {{"market_cap", 100.0}}},
        {"BBB", "2024-01-02", 10.0, {{"market_cap", 200.0}}},
        {"CCC", "2024-01-02", 10.0, {{"market_cap", 300.0}}},
        {"DDD", "2024-01-02", 10.0, {{"market_cap", 400.0}}},
        {"BBB", "2024-01-03", 11.0, {{"market_cap", 200.0}}},
        {"CCC", "2024-01-03", 12.0, {{"market_cap", 300.0}}},
        {"DDD", "2024-01-03", 13.0, {{"market_cap", 400.0}}},
        {"AAA", "2024-01-04", 20.0, {{"market_cap", 100.0}}},
        {"BBB", "2024-01-04", 11.0, {{"market_cap", 200.0}}},
        {"CCC", "2024-01-04", 12.0, {{"market_cap", 300.0}}},
        {"DDD", "2024-01-04", 13.0, {{"market_cap", 400.0}}},
    };
    ...
    EXPECT_EQ(result.status, std::string("SUCCESS")) << result.errorMessage;
    ASSERT_EQ(result.groupResult.groupReturns.size(), 2U);
    EXPECT_NEAR(result.groupResult.groupReturns[0], 0.10, 1e-9);
    EXPECT_NEAR(result.groupResult.groupReturns[1], 0.25, 1e-9);
    EXPECT_NEAR(result.groupResult.longShortReturn, -0.15, 1e-9);
}

TEST(FactorBacktestRegressionTest, ExecutorFutureReturnKeepsLastObservedCloseUntilDelistByDefault)
{
    const QString instanceId = QStringLiteral("size_future_return_keep_until_delist");
    factor::FactorInstanceInfo instanceInfo;
    instanceInfo.instanceId = instanceId.toStdString();
    instanceInfo.factorType = factor::FactorType::SIZE;
    instanceInfo.instanceName = "Size Future Return Keep Until Delist";
    instanceInfo.description = "Size Future Return Keep Until Delist";
    instanceInfo.isAvailable = true;
    instanceInfo.config = parseRuntimeCompatibleTestConfig(R"JSON({
        "factorType": "size",
        "calculation": {
    ...
    config.cachedBars = {
        ...
        {"CCC", "2024-01-02", 10.0, {{"market_cap", 300.0}}},
        {"DDD", "2024-01-02", 10.0, {{"market_cap", 400.0}}},
        {"AAA", "2024-01-03", 6.0, {{"market_cap", 100.0}}},
        {"BBB", "2024-01-03", 11.0, {{"market_cap", 200.0}}},
        {"CCC", "2024-01-03", 12.0, {{"market_cap", 300.0}}},
        {"DDD", "2024-01-03", 13.0, {{"market_cap", 400.0}}},
        {"BBB", "2024-01-04", 11.0, {{"market_cap", 200.0}}},
    ...
    EXPECT_EQ(result.status, std::string("SUCCESS")) << result.errorMessage;
    ASSERT_EQ(result.groupResult.groupReturns.size(), 2U);
    EXPECT_NEAR(result.groupResult.groupReturns[0], -0.15, 1e-9);
    EXPECT_NEAR(result.groupResult.groupReturns[1], 0.25, 1e-9);
    EXPECT_NEAR(result.groupResult.longShortReturn, -0.40, 1e-9);
}
...
TEST(FactorBacktestRegressionTest, ExecutorDateParallelGrowthFactorUsesIsolatedInstancesPerChunk)
{
    ...
    const QJsonObject configObject{
        {QStringLiteral("factorType"), factor::factorTypeIndex(factor::FactorType::GROWTH)},
        {QStringLiteral("majorCategory"), QStringLiteral("成长因子")},
        {QStringLiteral("dataRequirements"), QJsonObject{
        ...
        {QStringLiteral("calculation"), QJsonObject{
            {QStringLiteral("growthMetrics"), QJsonArray{static_cast<int>(factor::GrowthMetric::REVENUE_GROWTH)}},
            {QStringLiteral("growthWeights"), QJsonArray{1.0}},
            {QStringLiteral("frequency"), static_cast<int>(factor::DataFrequency::Daily)},
            {QStringLiteral("standardization"), static_cast<int>(factor::StandardizationMethod::None)},
            {QStringLiteral("lookbackWindow"), 2},
            {QStringLiteral("neutralizationEnabled"), true},
            {QStringLiteral("neutralizationMethod"), static_cast<int>(factor::NeutralizationMethod::IndustryMarketCap)}
        }},
        {QStringLiteral("boundaryRules"), QJsonObject{{QStringLiteral("minDataPoints"), 2}}}
    };
    const QByteArray configJson = QJsonDocument(configObject).toJson(QJsonDocument::Compact);

    const auto instanceInfo = makeFactorInstanceInfo(instanceId, QString::fromUtf8("成长因子"), configJson.constData());
    auto factorInstance = makeConfiguredFactor(configJson.constData());
    ASSERT_NE(factorInstance, nullptr);
    ...
    const std::unordered_map<std::string, std::vector<double>> closesBySymbol = {
        {"AAA", {10.0, 10.5, 11.0, 11.3}},
        {"BBB", {10.0, 10.3, 10.6, 10.9}},
        {"CCC", {10.0, 9.9, 9.8, 9.7}},
        {"DDD", {10.0, 10.1, 10.0, 10.2}}
    };
    const std::unordered_map<std::string, std::vector<double>> revenueBySymbol = {
        {"AAA", {100.0, 115.0, 132.0, 150.0}},
        {"BBB", {100.0, 110.0, 121.0, 133.0}},
        {"CCC", {100.0, 106.0, 112.0, 118.0}},
        {"DDD", {100.0, 104.0, 108.0, 112.0}}
    };
    const std::unordered_map<std::string, double> marketCapBySymbol = {
        {"AAA", 1500.0},
        {"BBB", 1200.0},
        {"CCC", 900.0},
        {"DDD", 800.0}
    };
    const std::unordered_map<std::string, double> industryBySymbol = {
        {"AAA", 10.0},
        {"BBB", 10.0},
        {"CCC", 20.0},
        {"DDD", 20.0}
    ...
    cachedBars.reserve(tradeDates.size() * closesBySymbol.size());
    for (size_t dateIndex = 0; dateIndex < tradeDates.size(); ++dateIndex) {
        for (const auto& [symbol, closeSeries] : closesBySymbol) {
            cachedBars.push_back({
                symbol,
                tradeDates[dateIndex],
                closeSeries[dateIndex],
                {
                    {"total_revenue", revenueBySymbol.at(symbol)[dateIndex]},
                    {"market_cap", marketCapBySymbol.at(symbol)},
                    {"industry_code", industryBySymbol.at(symbol)}
                }
            });
        }
    }
    ...
    auto threadPool = std::make_shared<foundation::thread::ThreadPoolExecutor>(4);
    FactorBacktestExecutor executor(instanceManager, threadPool, nullptr);
    const auto result = executor.execute(config);

    EXPECT_EQ(result.status, std::string("SUCCESS")) << result.errorMessage;
    EXPECT_TRUE(result.errorMessage.empty()) << result.errorMessage;
    EXPECT_FALSE(result.groupResult.groupReturns.empty());
    EXPECT_FALSE(result.icirResult.icSeries.empty());

    threadPool->shutdown();
}

TEST(FactorBacktestRegressionTest, ExecutorSkipsDatesWithInsufficientNeutralizationSamples)
{
    const QString instanceId = QStringLiteral("size_factor_neutralization_sparse_dates");
    const char* configJson = R"JSON({
        "factorType": "size",
        "majorCategory": "规模因子",
        "calculation": {
    ...
    BacktestConfig config = makeCachedBacktestConfig(instanceId.toStdString());
    config.startDate = "2024-01-02";
    config.endDate = "2024-01-03";
    config.forwardDays = 1;
    config.rebalanceDays = 1;
    config.numGroups = 2;

    const std::vector<std::string> symbols{"AAA", "BBB", "CCC", "DDD"};
    const std::vector<std::string> dates{"2024-01-02", "2024-01-03", "2024-01-04"};
    for (size_t dateIndex = 0; dateIndex < dates.size(); ++dateIndex) {
        for (size_t symbolIndex = 0; symbolIndex < symbols.size(); ++symbolIndex) {
            factor::CachedMarketBar bar;
            bar.symbol = symbols[symbolIndex];
            bar.tradeDate = dates[dateIndex];
            bar.close = 10.0 + static_cast<double>(dateIndex) + static_cast<double>(symbolIndex) * 0.2;
            bar.numericFields["industry_code"] = symbolIndex < 2 ? 10.0 : 20.0;

            if (dateIndex == 0) {
                if (symbolIndex < 2) {
                    bar.numericFields["market_cap"] = 100.0 + static_cast<double>(symbolIndex) * 30.0;
                }
            } else {
                bar.numericFields["market_cap"] = 100.0 + static_cast<double>(symbolIndex) * 40.0;
            }
...
TEST(FactorBacktestRegressionTest, ExecutorResearchRebalancesEvenWhenExecutionGateKeepsPreviousHoldings)
{
    const QString instanceId = QStringLiteral("size_factor_research_rebalance_independent_from_execution_gate");
    const char* configJson = R"JSON({
        "factorType": "size",
        "majorCategory": "规模因子",
        "calculation": {
    ...
    config.signalChangeThresholdStdMultiplier = 1000.0;
    config.marketEnvironmentProfile = factor::MarketEnvironmentProfile::GENERIC_EQUITY;
    ...
    const std::unordered_map<std::string, std::vector<double>> marketCapBySymbol{
        {"AAA", {100.0, 104.0, 104.0}},
        {"BBB", {101.0, 105.0, 105.0}},
        {"CCC", {102.0, 100.0, 100.0}},
        {"DDD", {103.0, 101.0, 101.0}}
    };
    ...
    for (size_t dateIndex = 0; dateIndex < dates.size(); ++dateIndex) {
        for (const auto& symbol : symbols) {
            ...
            bar.tradeDate = dates[dateIndex];
            bar.close = closesBySymbol.at(symbol)[dateIndex];
            bar.numericFields["market_cap"] = marketCapBySymbol.at(symbol)[dateIndex];
            config.cachedBars.push_back(std::move(bar));
        }
    }
    ...
    EXPECT_EQ(result.status, std::string("SUCCESS")) << result.errorMessage;
    EXPECT_TRUE(result.errorMessage.empty()) << result.errorMessage;
    ASSERT_EQ(result.rawLongShortSeries.size(), 2U);
    ASSERT_EQ(result.costAdjustedLongShortSeries.size(), 2U);
    EXPECT_NEAR(result.rawLongShortSeries[0], 0.2, 1e-9);
    EXPECT_NEAR(result.costAdjustedLongShortSeries[0], 0.2, 1e-9);
    EXPECT_GT(result.rawLongShortSeries[1], 0.4);
    EXPECT_LT(result.costAdjustedLongShortSeries[1], -0.4);
}
...
TEST(FactorBacktestRegressionTest, ExecutorGrowthFactorUsesLatestSparseFinancialObservations)
{
    ...
    const QJsonObject configObject{
        ...
        {QStringLiteral("majorCategory"), QStringLiteral("成长因子")},
        {QStringLiteral("dataRequirements"), QJsonObject{
            {QStringLiteral("required"), QJsonArray{QStringLiteral("total_revenue")}}
        }},
        {QStringLiteral("calculation"), QJsonObject{
            {QStringLiteral("growthMetrics"), QJsonArray{static_cast<int>(factor::GrowthMetric::REVENUE_GROWTH)}},
            {QStringLiteral("growthWeights"), QJsonArray{1.0}},
            {QStringLiteral("frequency"), static_cast<int>(factor::DataFrequency::Daily)},
            {QStringLiteral("standardization"), static_cast<int>(factor::StandardizationMethod::None)},
            {QStringLiteral("lookbackWindow"), 252},
            {QStringLiteral("neutralizationEnabled"), false}
        }},
        {QStringLiteral("boundaryRules"), QJsonObject{{QStringLiteral("minDataPoints"), 2}}}
    };
    ...
    BacktestConfig config = makeCachedBacktestConfig(instanceId.toStdString());
    config.startDate = "2024-04-30";
    config.endDate = "2024-05-31";
    config.forwardDays = 1;
    config.numGroups = 2;
    ...
    const std::unordered_map<std::string, std::vector<double>> closesBySymbol = {
        {"AAA", {10.0, 10.2, 10.4, 10.8, 11.1, 11.5}},
        {"BBB", {10.0, 10.1, 10.3, 10.5, 10.6, 10.8}},
        {"CCC", {10.0, 10.0, 10.1, 10.2, 10.2, 10.3}},
        {"DDD", {10.0, 9.9, 9.8, 9.7, 9.6, 9.5}}
    };
    const std::unordered_map<std::string, std::pair<double, double>> revenueBySymbol = {
        {"AAA", {100.0, 150.0}},
        {"BBB", {100.0, 130.0}},
        {"CCC", {100.0, 110.0}},
        {"DDD", {100.0, 90.0}}
    ...
    for (size_t dateIndex = 0; dateIndex < tradeDates.size(); ++dateIndex) {
        for (const auto& [symbol, closeSeries] : closesBySymbol) {
            ...
            if (tradeDates[dateIndex] == "2024-02-29") {
                numericFields["total_revenue"] = revenueBySymbol.at(symbol).first;
            } else if (tradeDates[dateIndex] == "2024-04-30") {
                numericFields["total_revenue"] = revenueBySymbol.at(symbol).second;
            }
...
TEST(FactorBacktestRegressionTest, GrowthBoundaryRulesIncludeHistoryDepthAndLag)
{
    const char* configJson = R"JSON({
        "factorType": 4,
        "majorCategory": "成长因子",
    ...
    const auto rules = factorInstance->getBoundaryRules();
    EXPECT_EQ(rules.minDataPoints, 10);
}

TEST(FactorBacktestRegressionTest, GrowthBoundaryRulesMatchSueSeriesDepth)
{
    const char* configJson = R"JSON({
        "factorType": 4,
        "majorCategory": "成长因子",
    ...
    const auto rules = factorInstance->getBoundaryRules();
    EXPECT_EQ(rules.minDataPoints, 8);
}

TEST(FactorBacktestRegressionTest, ExecutorAppliesFactorWarmupTradingDateTrim)
{
    const char* configJson = R"JSON({
        "factorType": 4,
        "majorCategory": "成长因子",
...
TEST(FactorBacktestRegressionTest, ExecutorBuildsNumericIndustryBucketsFromCachedDatasetRows)
{
    ...
    const QString instanceId = QStringLiteral("size_string_industry_runtime");
    factor::FactorInstanceInfo instanceInfo;
    instanceInfo.instanceId = instanceId.toStdString();
    instanceInfo.factorType = factor::FactorType::SIZE;
    instanceInfo.instanceName = "Size String Industry Runtime";
    instanceInfo.description = "Size String Industry Runtime";
    instanceInfo.isAvailable = true;
    instanceInfo.config = parseRuntimeCompatibleTestConfig(R"JSON({
        "factorType": "size",
        "calculation": {
    ...
    const QStringList tradeDates{
        QStringLiteral("2024-01-02"),
        QStringLiteral("2024-01-03"),
        QStringLiteral("2024-01-04"),
        QStringLiteral("2024-01-05")
    };

    QVariantList rows;
    rows.reserve(stockCodes.size() * tradeDates.size());
    for (int dateIndex = 0; dateIndex < tradeDates.size(); ++dateIndex) {
        const QString& tradeDate = tradeDates.at(dateIndex);
        for (int symbolIndex = 0; symbolIndex < stockCodes.size(); ++symbolIndex) {
            const QString& symbol = stockCodes.at(symbolIndex);
            const double closeValue = 10.0 + static_cast<double>(dateIndex) * 0.4 + static_cast<double>(symbolIndex) * 0.2;
            const double circulatingMarketCap = 100.0 + static_cast<double>(symbolIndex) * 25.0;
            const double marketCap = circulatingMarketCap * 1.2;
            rows.append(QVariantMap{
                {QStringLiteral("symbol"), symbol},
                {QStringLiteral("trade_date"), tradeDate},
                {QStringLiteral("close"), closeValue},
                {QStringLiteral("pre_adjust_factor"), 1.0},
                {QStringLiteral("post_adjust_factor"), 1.0},
                {QStringLiteral("circulating_market_cap"), circulatingMarketCap},
                {QStringLiteral("market_cap"), marketCap},
                {QStringLiteral("industry_code"), symbolIndex < 2 ? QStringLiteral("SW_01") : QStringLiteral("SW_02")}
            });
        }
    }
    ...
    config.rebalanceDays = 1;
    config.numGroups = 2;
    config.allowedStockCodes = {"AAA", "BBB", "CCC", "DDD"};
...
TEST(FactorBacktestRegressionTest, ExecutorUsesFinancialRowsWithDisclosureDateFromCachedDataset)
{
    auto& cache = DataServiceCache::getInstance();
    ASSERT_TRUE(cache.initializeCache());
    cache.clearAllCache();

    const QString instanceId = QStringLiteral("growth_dataset_disclosure_date_runtime");
    const QJsonObject configObject{
        {QStringLiteral("factorType"), factor::factorTypeIndex(factor::FactorType::GROWTH)},
        {QStringLiteral("majorCategory"), QStringLiteral("成长因子")},
        {QStringLiteral("dataRequirements"), QJsonObject{
    ...
    const QStringList tradeDates{
        QStringLiteral("2024-02-14"),
        QStringLiteral("2024-02-15"),
        QStringLiteral("2024-05-14"),
        QStringLiteral("2024-05-15"),
        QStringLiteral("2024-05-16"),
        QStringLiteral("2024-05-17")
    };
    ...
    for (int dateIndex = 0; dateIndex < tradeDates.size(); ++dateIndex) {
        for (int symbolIndex = 0; symbolIndex < stockCodes.size(); ++symbolIndex) {
            const QString& symbol = stockCodes.at(symbolIndex);
            const double closeValue = 10.0 + static_cast<double>(dateIndex) * 0.5 + static_cast<double>(symbolIndex) * 0.2;
            rows.append(QVariantMap{
                {QStringLiteral("symbol"), symbol},
                {QStringLiteral("trade_date"), tradeDates.at(dateIndex)},
                {QStringLiteral("close"), closeValue},
                {QStringLiteral("pre_adjust_factor"), 1.0},
                {QStringLiteral("post_adjust_factor"), 1.0}
            });
    ...
    const std::array<std::pair<QString, std::array<double, 4>>, 2> revenueSnapshots{{
        {QStringLiteral("2024-02-15"), {100.0, 100.0, 100.0, 100.0}},
        {QStringLiteral("2024-05-15"), {150.0, 130.0, 110.0, 90.0}}
    }};
    for (const auto& [disclosureDate, values] : revenueSnapshots) {
        for (int symbolIndex = 0; symbolIndex < stockCodes.size(); ++symbolIndex) {
            rows.append(QVariantMap{
                {QStringLiteral("symbol"), stockCodes.at(symbolIndex)},
                {QStringLiteral("disclosure_date"), disclosureDate},
                {QStringLiteral("report_date"), disclosureDate},
                {QStringLiteral("data_type"), QStringLiteral("financial")},
                {QStringLiteral("total_revenue"), values[static_cast<size_t>(symbolIndex)]}
            });
        }
    }
    ...
    BacktestConfig config = makeCachedBacktestConfig(instanceId.toStdString());
    config.datasetId = datasetId;
    config.startDate = "2024-05-15";
    config.endDate = "2024-05-16";
    config.forwardDays = 1;
    config.rebalanceDays = 1;
    config.numGroups = 2;
    config.allowedStockCodes = {"AAA", "BBB", "CCC", "DDD"};
...
TEST(FactorBacktestRegressionTest, ArrowMarketDataMergesDuplicateRowsBySymbolAndTradeDate)
{
    const double nan = std::numeric_limits<double>::quiet_NaN();
    const std::vector<factor::CachedMarketBar> bars = {
        {"AAA", "2024-01-08 15:00:00", 10.0, {{"close", 10.0}}},
        {"AAA", "2024-01-08", nan, {{"post_adjust_factor", 1.1}}},
        {"AAA", "2024-01-09 15:00:00", 11.0, {{"close", 11.0}}},
        {"AAA", "2024-01-09", nan, {{"post_adjust_factor", 1.2}}},
    };

    const auto data = factor::ArrowMarketData::fromCachedBars(bars);
    ASSERT_NE(data, nullptr);
    EXPECT_EQ(data->rowCount(), 2);

    const auto closeSeries = data->getSeries("AAA", "2024-01-08", "2024-01-09", "close");
    const auto adjustSeries = data->getSeries("AAA", "2024-01-08", "2024-01-09", "post_adjust_factor");

    ASSERT_EQ(closeSeries.size(), 2U);
    ASSERT_EQ(adjustSeries.size(), 2U);
    EXPECT_EQ(closeSeries[0].date, std::string("2024-01-08"));
    EXPECT_EQ(closeSeries[1].date, std::string("2024-01-09"));
    EXPECT_EQ(adjustSeries[0].date, std::string("2024-01-08"));
    EXPECT_EQ(adjustSeries[1].date, std::string("2024-01-09"));
    EXPECT_DOUBLE_EQ(closeSeries[0].value, 10.0);
    EXPECT_DOUBLE_EQ(closeSeries[1].value, 11.0);
    EXPECT_DOUBLE_EQ(adjustSeries[0].value, 1.1);
    EXPECT_DOUBLE_EQ(adjustSeries[1].value, 1.2);
}
...
TEST(FactorBacktestRegressionTest, GroupBacktestAggregateIgnoresExecutionSignalGateForResearchGroups)
{
    const std::vector<CalculationResult> factorResults = {
        makeCalculationResult("2024-01-08", {{"AAA", 4.0}, {"BBB", 3.0}, {"CCC", 2.0}, {"DDD", 1.0}}),
        makeCalculationResult("2024-01-09", {{"AAA", 4.01}, {"BBB", 3.01}, {"CCC", 2.01}, {"DDD", 1.01}}),
    };
    const std::vector<CalculationResult> returnResults = {
        makeCalculationResult("2024-01-08", {{"AAA", 0.10}, {"BBB", 0.05}, {"CCC", -0.02}, {"DDD", -0.04}}),
        makeCalculationResult("2024-01-09", {{"AAA", -0.10}, {"BBB", -0.05}, {"CCC", 0.02}, {"DDD", 0.04}}),
    };
    ...
    ASSERT_TRUE(summary.hasValidGroup);
    ASSERT_EQ(summary.longShortReturnsByDate.size(), 2U);
    EXPECT_NEAR(summary.longShortReturnsByDate[0], 0.105, 1e-9);
    EXPECT_NEAR(summary.longShortReturnsByDate[1], -0.105, 1e-9);
    EXPECT_NEAR(summary.groupResult.groupReturns[0], 0.0, 1e-9);
    EXPECT_NEAR(summary.groupResult.groupReturns[1], 0.0, 1e-9);
}
...
TEST(FactorBacktestRegressionTest, MarketBarBacktestReadyFieldsIncludeIndustryCode)
{
    const QStringList fields = factor::bridge::marketBarBacktestReadyFields().orderedValues();
    EXPECT_TRUE(fields.contains(QStringLiteral("industry_code")));
}
    ...
    factor::ConfigurableFactorTestAccess::loadConfig(
        ...
        jsonFacadeFromObjectForTest(QJsonObject{
            ...
            {QStringLiteral("calculation"), QJsonObject{
                ...
                {QStringLiteral("frequency"), 0},
                {QStringLiteral("lookbackWindow"), 3},
                {QStringLiteral("laggedEnabled"), false},
                {QStringLiteral("standardization"), static_cast<int>(factor::StandardizationMethod::None)}
            }}
        }));
...
TEST(FactorBacktestRegressionTest, ConfigurableGrowthFactorWeightsAffectResult)
{
    ...
    EXPECT_GT(revenueWeightedResult.values.at("AAA"), revenueWeightedResult.values.at("BBB"));
    EXPECT_LT(roeWeightedResult.values.at("AAA"), roeWeightedResult.values.at("BBB"));
...
TEST(FactorBacktestRegressionTest, ConfigurableGrowthFactorHandlesSparseFinancialSeries)
{
    ...
    ASSERT_TRUE(result.dataStatus.isValid());
    ASSERT_EQ(result.values.size(), 1U);
    EXPECT_TRUE(result.values.find("AAA") != result.values.end());
    EXPECT_TRUE(result.values.find("BBB") == result.values.end());
}
...
TEST(FactorBacktestRegressionTest, BuildFactorSupportMapRejectsNeutralizationFieldWithoutNonNullValues)
{
    auto& cache = DataServiceCache::getInstance();
    ASSERT_TRUE(cache.initializeCache());
    cache.clearAllCache();

    const QString instanceId = QStringLiteral("liquidity_neutralization_empty_industry_instance");
    factor::FactorInstanceInfo instanceInfo;
    instanceInfo.instanceId = instanceId.toStdString();
    instanceInfo.factorType = factor::FactorType::CUSTOM;
    instanceInfo.instanceName = "Liquidity Neutralization Empty Industry";
    instanceInfo.description = "Liquidity Neutralization Empty Industry";
    instanceInfo.isAvailable = true;
    instanceInfo.config = parseRuntimeCompatibleTestConfig(R"JSON({
        "factorType": "liquidity",
        "calculation": {
    ...
    const int datasetId = storeSupportMapDataset(
        rows,
        QStringList{QStringLiteral("close"), QStringLiteral("turnover_rate"), QStringLiteral("industry_code"), QStringLiteral("market_cap")},
        QStringList{QStringLiteral("AAA")},
        QStringLiteral("2024-01-02"),
        QStringLiteral("2024-01-02"));
    ASSERT_GT(datasetId, 0);
    ...
    const QVariantMap cacheSnapshot{
        {QStringLiteral("availableFields"), QStringList{QStringLiteral("close"), QStringLiteral("turnover_rate"), QStringLiteral("industry_code"), QStringLiteral("market_cap")}},
        {QStringLiteral("tradeDateCount"), 1},
        {QStringLiteral("fieldDiagnostics"), QVariantMap{
            {QStringLiteral("turnover_rate"), QVariantMap{{QStringLiteral("nonNullCount"), 1}, {QStringLiteral("latestDateNonNullCount"), 1}}},
            {QStringLiteral("industry_code"), QVariantMap{{QStringLiteral("nonNullCount"), 0}, {QStringLiteral("latestDateNonNullCount"), 0}}},
            {QStringLiteral("market_cap"), QVariantMap{{QStringLiteral("nonNullCount"), 1}, {QStringLiteral("latestDateNonNullCount"), 1}}}
        }}
    };
    ...
    EXPECT_EQ(supportInfo.value("category").toString(), QStringLiteral("missing-field"));
    EXPECT_EQ(factor::bridge::test::supportInfoRuntimeTypeForTest(supportInfo), factor::FactorType::LIQUIDITY);
    EXPECT_EQ(supportInfo.value("missingFields").toList(), QVariantList{QStringLiteral("industry_code")});
    EXPECT_TRUE(supportInfo.value("reason").toString().contains(QStringLiteral("无有效非空值")));
...
TEST(FactorBacktestRegressionTest, BuildFactorSupportMapUsesMomentumRuntimeWarmupTradingDays)
{
    auto& cache = DataServiceCache::getInstance();
    ASSERT_TRUE(cache.initializeCache());
    cache.clearAllCache();

    const QString instanceId = QStringLiteral("momentum_runtime_warmup_instance");
    factor::FactorInstanceInfo instanceInfo;
    instanceInfo.instanceId = instanceId.toStdString();
    instanceInfo.factorType = factor::FactorType::MOMENTUM;
    instanceInfo.instanceName = "Momentum Runtime Warmup";
    instanceInfo.description = "Momentum Runtime Warmup";
    instanceInfo.isAvailable = true;
    instanceInfo.config = parseRuntimeCompatibleTestConfig(R"JSON({
        "factorType": "momentum",
        "calculation": {
    ...
    auto factorInstance = factor::MomentumFactor::create(instanceInfo, nullptr);
    ASSERT_NE(factorInstance, nullptr);
    EXPECT_EQ(factorInstance->getBoundaryRules().minDataPoints, 281);

    QVariantList rows;
    const QDate startDate(2024, 1, 2);
    for (int offset = 0; offset < 58; ++offset) {
        const QString tradeDate = startDate.addDays(offset).toString(Qt::ISODate);
        rows.append(QVariantMap{{"symbol", "AAA"},
                                {"trade_date", tradeDate},
                                {"close", 10.0 + static_cast<double>(offset) * 0.1},
                                {"post_adjust_factor", 1.0}});
    }
    const int datasetId = storeSupportMapDataset(
        rows,
        QStringList{QStringLiteral("close"), QStringLiteral("post_adjust_factor")},
        QStringList{QStringLiteral("AAA")},
        startDate.toString(Qt::ISODate),
        startDate.addDays(57).toString(Qt::ISODate));
    ASSERT_GT(datasetId, 0);
    ...
    EXPECT_EQ(factor::bridge::test::supportInfoRuntimeTypeForTest(supportInfo), factor::FactorType::MOMENTUM);
    EXPECT_TRUE(supportInfo.value("reason").toString().contains(QStringLiteral("58")));
    EXPECT_TRUE(supportInfo.value("reason").toString().contains(QStringLiteral("281")));
...
TEST(FactorBacktestRegressionTest, StringFieldAccessorTreatsEmptyStringAsMissingValue)
{
    const QVariantMap record{
        {QStringLiteral("symbol"), QStringLiteral("000001.SZ")},
        {QStringLiteral("industry_code"), QStringLiteral("   ")}
    };

    EXPECT_TRUE(factor::bridge::Accessors::Industry.has(record));
    EXPECT_FALSE(factor::bridge::Accessors::Industry.hasValue(record));
    EXPECT_TRUE(factor::bridge::Accessors::Symbol.has(record));
    EXPECT_TRUE(factor::bridge::Accessors::Symbol.hasValue(record));
}
...
TEST(FactorBacktestRegressionTest, QtMySqlDatabaseSupportsMultiplePositionalBindings)
{
    auto database = astock::database::DatabaseConnectionManager::instance().getDatabase();
    if (!database) {
        GTEST_SKIP() << "database unavailable";
    ...
    const auto result = database->executeQuery(
        QStringLiteral("SELECT 1 FROM DUAL WHERE ? = 11 AND ? = 22"),
        {
            {QStringLiteral("__pos_000000"), 11},
            {QStringLiteral("__pos_000001"), 22}
        });
