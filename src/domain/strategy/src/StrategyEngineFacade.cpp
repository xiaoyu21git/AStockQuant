...
bool StrategyEngine::prepareMarketData()
{
    ...
    if (v) {
        ...
        // 预注入全市场标的到 MarketDataService (EOD评估需要)
        for (const auto& sym : v->symbolStrings())
            domain::market::MarketDataService::instance().liveData(sym);
        INTERNAL_INFO_STREAM << "[Engine] MarketDataService 已注入 " << v->symbolStrings().size() << " 标的";
        return true;
    }
