...
public:
    explicit FileProvider(const std::string& /*config*/) {}
    ...
    bool subscribe_kline(std::uint32_t, std::uint32_t) override { return true; }
    bool unsubscribe_kline(std::uint32_t, std::uint32_t) override { return true; }
