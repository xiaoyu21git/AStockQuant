...
private:
    ...
    bool fetchFromJuejin(QVariantList& data, QString& errorMessage) {
        try {
            ...
            // 构建Python命令
            QString pythonScript = QDir::currentPath() + "/tools/import_from_juejin_single.py";
            
            // 检查Python脚本是否存在，如果不存在则创建
            if (!QFile::exists(pythonScript)) {
                createJuejinFetchScript(pythonScript);
            }
    ...
    void createJuejinFetchScript(const QString& scriptPath) {
...
def main():
    parser = argparse.ArgumentParser(description='从掘金获取股票数据')
    ...
    start_date_str = args.start_date or DEFAULT_START_DATE.strftime("%Y-%m-%d")
    end_date_str = args.end_date or DEFAULT_END_DATE.strftime("%Y-%m-%d")
