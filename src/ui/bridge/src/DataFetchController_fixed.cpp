...
void DataFetchController::cleanDataAsync(const QVariantMap& rules)
{
    ...
    // 从DataManager获取当前缓存的数据 - 尝试多种可能的缓存键
    QVariantList data;
    
    // 首先尝试标准缓存键 - 这是loadFromDatabase成功时存储的键
    data = DataManager::instance()->getData("current_stock_data");
    if (!data.isEmpty()) {
        qDebug() << "DataFetchController::cleanDataAsync: Found data in 'current_stock_data' key, count:" << data.size();
    }
    ...
    if (data.isEmpty()) {
        ...
        data = DataManager::instance()->getData(cacheKey);
        qDebug() << "Trying DataServiceCache compatible key:" << cacheKey << "found:" << data.size() << "items";
    }
    
    if (data.isEmpty()) {
        // 尝试数据集缓存键
        QString datasetKey = QString("dataset_%1_%2").arg(m_currentStartDate).arg(m_currentEndDate);
        data = DataManager::instance()->getData(datasetKey);
        qDebug() << "Trying dataset key:" << datasetKey << "found:" << data.size() << "items";
    }
    
    // 如果以上都失败，尝试从DataServiceCache通过ID获取数据集
    if (data.isEmpty()) {
        qDebug() << "DataFetchController::cleanDataAsync: No data found in traditional cache keys, trying ID-based cache...";
        
        // 获取DataServiceCache实例
        DataServiceCache& cache = DataServiceCache::getInstance();
        
        // 查询包含"last_cleaning_result"或类似标签的数据集
        DataServiceCache::DataSetQuery query;
        query.tags = QStringList{"last_cleaning_result", "cleaning_result", "dataset"};
        
        auto dataSetInfos = cache.queryDataSets(query);
        qDebug() << "Found" << dataSetInfos.size() << "datasets with relevant tags";
        ...
        if (!dataSetInfos.isEmpty()) {
            ...
            qDebug() << "Loading latest dataset ID:" << latestDataId << "name:" << dataSetInfos.first().displayName;
            
            data = cache.getDataSetById(latestDataId);
            if (!data.isEmpty()) {
                qDebug() << "Successfully loaded dataset via ID, count:" << data.size();
