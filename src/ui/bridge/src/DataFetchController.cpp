...
void DataFetchController::loadFromDatabase(const QString& symbol, const QString& startDate, const QString& endDate)
{
    ...
    QVariantList cachedData;
    {
        ...
        if (!cachedData.isEmpty()) {
            qDebug() << "DataFetchController::loadFromDatabase: Using DataServiceCache for"
                     << (actualSymbol.isEmpty() ? "ALL" : actualSymbol)
...
void DataFetchController::onDataLoadCompleted(bool success, const QString& message, const QVariantList& data)
{
    ...
    if (success) {
        ...
        if (m_previewModel) {
            ...
            qDebug() << "PreviewDataModel updated with" << dataVector.size() << "stock summaries (data load)";
        } else {
...
void DataFetchController::onDataCleaningCompleted(bool success, const QString& message, const QVariantList& cleanedData)
{
    ...
    if (success) {
        ...
        if (m_previewModel) {
            ...
            m_previewModel->updateData(dataVector);
            qDebug() << "PreviewDataModel updated with" << dataVector.size() << "stock summaries (data cleaning)";
        } else {
        ...
        if (cache.isCacheEnabled() && !cleanedData.isEmpty()) {
            ...
            if (!m_currentStartDate.isEmpty() || !m_currentEndDate.isEmpty()) {
                dataSetInfo.description += QString("，范围: %1 ~ %2").arg(m_currentStartDate, m_currentEndDate);
            }

            const QString ruleSummary = summarizeRuleNames(m_pendingRules);
            ...
            if (!tradeDates.isEmpty()) {
                ...
                dataSetInfo.startDate = QDate::fromString(m_currentStartDate, "yyyy-MM-dd");
                dataSetInfo.endDate = QDate::fromString(m_currentEndDate, "yyyy-MM-dd");
            }

            const int dataSetId = cache.storeDataSet(cleanedData, dataSetInfo);
            if (dataSetId > 0) {
                qDebug() << "DataFetchController::onDataCleaningCompleted: 已保存清洗结果数据集:" << dataSetId
                         << dataSetInfo.displayName;
            } else {
                qWarning() << "DataFetchController::onDataCleaningCompleted: 保存清洗结果数据集失败";
            }
        }
...
void DataFetchController::refreshAllCacheInfos()
{
    ...
    // 然后添加缓存键（包括所有键）
    for (const QString& key : cacheKeys) {
        // 如果该 key 已经由数据集信息表示，则不再重复展示，避免同一份数据出现两条。
        if (representedCacheKeys.contains(key)) {
            qDebug() << "Skipping duplicate cache key already represented by dataset:" << key;
            continue;
        }
...
{
    qDebug() << "DataFetchController::cleanDataFromDataSetId() - ID:" << dataId;
...
{
    qDebug() << "DataFetchController::cleanDataFromCacheKey() - Key:" << cacheKey;
...
// 指数成分股相关方法实现
void DataFetchController::loadIndexConstituents(const QString& indexSymbol)
{
    qDebug() << "DataFetchController::loadIndexConstituents() - Loading index constituents for:" << indexSymbol;
    
    if (indexSymbol.isEmpty()) {
        qDebug() << "DataFetchController::loadIndexConstituents: Index symbol is empty";
        emit dataFetchError("指数代码不能为空");
        return;
    }
    ...
    updateStatus(QString("正在加载指数 %1 的成分股...").arg(indexSymbol), 0);
    
    // 直接调用DataService的方法
    // 注意：DataService::loadIndexConstituents会发出queryProgress/queryCompleted信号
    // 这些信号已经通过onDataLoadProgress/onDataLoadCompleted连接到控制器
    m_dataService->loadIndexConstituents(indexSymbol);
}

QVariantList DataFetchController::getAvailableIndices()
{
    qDebug() << "DataFetchController::getAvailableIndices()";
    
    // 直接调用DataService的方法
    QVariantList indices = m_dataService->getAvailableIndices();
    
    qDebug() << "Found" << indices.size() << "available indices";
    return indices;
}
...
void DataFetchController::fetchDataByType(const QString& dataSource,
    ...
    qDebug() << "DataFetchController::fetchDataByType - Forwarding request to DataService";
