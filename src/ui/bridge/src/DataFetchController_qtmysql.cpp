// DataFetchController_qtmysql.cpp - 使用QtMySQLDatabase封装类的版本
...
QVariantList DataFetchController::cleanData(const QVariantList& data, const QVariantMap& rules)
{
    ...
    if (rules.contains("formatValidation") && rules["formatValidation"].toBool()) {
        ...
        rule.parameters["datePattern"] = "^\\d{4}-\\d{2}-\\d{2}$";
        rule.enabled = true;
        cleaningRules.append(rule);
    }
    ...
    qDebug() << "Data cleaning completed: original" << data.size() << "-> cleaned" << cleanedData.size();
