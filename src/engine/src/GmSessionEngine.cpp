...
std::vector<GmTickData> GmSessionEngine::fetchMinuteHistory(
    const std::string& symbol, int64_t startEpoch, int64_t endEpoch)
{
    std::vector<GmTickData> result;
    std::string gm = toGmSymbol(symbol);
    if (gm.empty()) return result;

    char s[32], e[32];
    time_t st = static_cast<time_t>(startEpoch);
    time_t et = static_cast<time_t>(endEpoch);
    strftime(s, sizeof(s), "%Y-%m-%d %H:%M:%S", localtime(&st));
    strftime(e, sizeof(e), "%Y-%m-%d %H:%M:%S", localtime(&et));

    auto* bars = ::history_bars(gm.c_str(), "60s", s, e, 0, nullptr, true, nullptr);
    if (!bars || bars->status() || !bars->count()) {
        if (bars) bars->release();
        return result;
    }

    result.reserve(static_cast<size_t>(bars->count()));
    for (int i = 0; i < bars->count(); ++i) {
        ...
        td.high       = b.high;
        td.low        = b.low;
        td.cumVolume  = static_cast<double>(b.volume);
        td.cumAmount  = b.amount;
        td.lastVolume = static_cast<double>(b.volume);
        td.isAuction  = false;
        td.tradeType  = 0;
        ...
        if (b.eob > 0) {
            ...
            td.createdAt  = b.bob + 60;
        }
        td.tradingDay = 0; // onTick 会从 td.createdAt 推导 tradingDay
...
std::optional<GmTickData> GmSessionEngine::fetchDailyBar(
    const std::string& symbol, int64_t tradingDay)
{
    std::string gm = toGmSymbol(symbol);
    if (gm.empty()) return std::nullopt;

    // tradingDay 是 YYYYMMDD 格式，转为 YYYY-MM-DD
    int y = static_cast<int>(tradingDay / 10000);
    int m = static_cast<int>((tradingDay % 10000) / 100);
    int d = static_cast<int>(tradingDay % 100);
    char dateStr[16];
    snprintf(dateStr, sizeof(dateStr), "%04d-%02d-%02d", y, m, d);

    auto* bars = ::history_bars(gm.c_str(), "1d", dateStr, dateStr, 0, nullptr, true, nullptr);
    if (!bars || bars->status() || !bars->count()) {
        if (bars) bars->release();
        return std::nullopt;
    }
